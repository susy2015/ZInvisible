...
int main(int argc, char* argv[])
{
    ...
    // y axis limits
    double massMinY = 5.0e-2;
    double massMaxY = 1.0e5;
    int minJets = 0;
    int maxJets = 11;
    // mass in GeV
    ...
    std::string label_dR_PromptPhotonGenParton      = "#DeltaR(PromptPhoton, GenParton)";
    ...
    std::string label_min_dR_PromptPhotonGenParton  = "min #DeltaR(PromptPhoton, GenParton)";
    ...
    PDS dsData_SingleMuon("Data",         fileMap["Data_SingleMuon"], "Pass_trigger_muon",   "");
    ...
    */
    ...
    // Datasetsummaries we are using                                                                                                        
    // no weight (genWeight deals with negative weights); also add btag weights here                                                        
    PDS dsData_SingleMuon("Data",         fileMap[MuonDataset],  "Pass_trigger_muon",   "");
    ...
    auto makeStackMC_DiLepton = [&](const std::string& cuts, const std::string& weights, const std::string& DYweight = "")
    {
        ...
        // put MC with Z mass peak on top and other MC on bottom
        // DY, Diboson + WZ, Rare + TTZ, TTbar, SingleTopZinv
        std::vector<std::vector<PDS>> StackMC = {{dsDY}, {dsDiboson, dsWZ}, {dsRare, dsTTZ}, {dsTTbar}, {dsSingleTopZinv}};
        return StackMC;
    };
    
    auto makeStackMC_DiLepton_Normalization = [&](const std::string& cuts, const std::string& weights, const bool split = false)
    {
        PDS dsDY(               "DY",                   fileMap["DYJetsToLL" + yearTag],      cuts,   weights);
        PDS dsDY_ZToLL(         "ZToLL",                fileMap["DYJetsToLL" + yearTag],      cuts,   weights);
        PDS dsTTbar(            "t#bar{t}",             fileMap["TTbarNoHad" + yearTag],      cuts,   weights + ISRWeight + topptWeight);
        PDS dsTTbar_NoZToLL(    "NoZToLL",              fileMap["TTbarNoHad" + yearTag],      cuts,   weights + ISRWeight + topptWeight);
        PDS dsSingleTopZinv(    "Single t",             fileMap["SingleTopZinv" + yearTag],   cuts,   weights);
    ...
    {
        // jet cleaning tag 
        std::string cleanTag   = "_drLeptonCleaned";
        ...
        SAT_Pass_lowDM_Loose  = ";SAT_Pass_lowDM_Loose"     + varSuffix;
        SAT_Pass_highDM_Loose = ";SAT_Pass_highDM_Loose"    + varSuffix;
        SAT_Pass_lowDM_Mid    = ";SAT_Pass_lowDM_Mid"       + varSuffix;
        SAT_Pass_highDM_Mid   = ";SAT_Pass_highDM_Mid"      + varSuffix;
        SAT_Pass_lowDM_Tight  = ";SAT_Pass_lowDM_Tight"     + varSuffix;
    
        // --- Normalization: bin in NB and NSV --- //
        std::map<std::string, std::string> lepton_var_map = {
        ...
        for (auto& cut : vec_norm_cuts_low_dm)
        {
            map_norm_cuts_low_dm[cut] = ";" + SusyUtility::parseCuts(cut, lepton_var_map);
        }
        for (auto& cut : vec_norm_cuts_high_dm)
        {
        ...
        if (doDataMCElectron)
        {
            ...
            for (const auto& cut : map_norm_cuts_low_dm)
            {
                ...
                std::vector<std::vector<PDS>> StackMC_Electron_LowDM_Normalization_noZMassCut_split = makeStackMC_DiLepton_Normalization(  "passElecZinvSel" + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned + cut.second, ElectronWeights, true);
                // bestRecoZM
                ...
                PDC dcMC_Electron_LowDM_Normalization_bestRecoZM_split(  "stack",  "bestRecoZM", StackMC_Electron_LowDM_Normalization_noZMassCut_split);
                vh.push_back(PHS("DataMC_Electron_LowDM_bestRecoZM_50to250_" + cut.first + histSuffix,                      {dcData_Electron_LowDM_bestRecoZM,   dcMC_Electron_LowDM_bestRecoZM},                       {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Electron_LowDM_bestRecoZM_0to400_" + cut.first + histSuffix,                       {dcData_Electron_LowDM_bestRecoZM,   dcMC_Electron_LowDM_bestRecoZM},                       {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Electron_LowDM_Normalization_bestRecoZM_50to250_" + cut.first + histSuffix,        {dcData_Electron_LowDM_bestRecoZM,   dcMC_Electron_LowDM_Normalization_bestRecoZM},         {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Electron_LowDM_Normalization_bestRecoZM_0to400_" + cut.first + histSuffix,         {dcData_Electron_LowDM_bestRecoZM,   dcMC_Electron_LowDM_Normalization_bestRecoZM},         {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Electron_LowDM_Normalization_bestRecoZM_split_50to250_" + cut.first + histSuffix,  {dcData_Electron_LowDM_bestRecoZM,   dcMC_Electron_LowDM_Normalization_bestRecoZM_split},   {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Electron_LowDM_Normalization_bestRecoZM_split_0to400_" + cut.first + histSuffix,   {dcData_Electron_LowDM_bestRecoZM,   dcMC_Electron_LowDM_Normalization_bestRecoZM_split},   {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
            ...
            for (const auto& cut : map_norm_cuts_high_dm)
            {
                ...
                std::vector<std::vector<PDS>> StackMC_Electron_HighDM_Normalization_noZMassCut_split = makeStackMC_DiLepton_Normalization(  "passElecZinvSel" + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned + cut.second, ElectronWeights, true);
                // bestRecoZM
                ...
                PDC dcMC_Electron_HighDM_Normalization_bestRecoZM_split(    "stack",  "bestRecoZM", StackMC_Electron_HighDM_Normalization_noZMassCut_split);
                vh.push_back(PHS("DataMC_Electron_HighDM_bestRecoZM_50to250_" + cut.first + histSuffix,                     {dcData_Electron_HighDM_bestRecoZM,   dcMC_Electron_HighDM_bestRecoZM},                     {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Electron_HighDM_bestRecoZM_0to400_" + cut.first + histSuffix,                      {dcData_Electron_HighDM_bestRecoZM,   dcMC_Electron_HighDM_bestRecoZM},                     {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Electron_HighDM_Normalization_bestRecoZM_50to250_" + cut.first + histSuffix,       {dcData_Electron_HighDM_bestRecoZM,   dcMC_Electron_HighDM_Normalization_bestRecoZM},       {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Electron_HighDM_Normalization_bestRecoZM_0to400_" + cut.first + histSuffix,        {dcData_Electron_HighDM_bestRecoZM,   dcMC_Electron_HighDM_Normalization_bestRecoZM},       {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Electron_HighDM_Normalization_bestRecoZM_split_50to250_" + cut.first + histSuffix, {dcData_Electron_HighDM_bestRecoZM,   dcMC_Electron_HighDM_Normalization_bestRecoZM_split}, {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Electron_HighDM_Normalization_bestRecoZM_split_0to400_" + cut.first + histSuffix,  {dcData_Electron_HighDM_bestRecoZM,   dcMC_Electron_HighDM_Normalization_bestRecoZM_split}, {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
            ...
            // MC
            // no Z mass or Pt cuts
            std::vector<std::vector<PDS>> StackMC_Electron_LowDM_noZMassCut_noZPtCut                = makeStackMC_DiLepton(                "passElecZinvNoZPtCut"  + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, ElectronWeights);
            std::vector<std::vector<PDS>> StackMC_Electron_HighDM_noZMassCut_noZPtCut               = makeStackMC_DiLepton(                "passElecZinvNoZPtCut"  + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, ElectronWeights);
            std::vector<std::vector<PDS>> StackMC_Electron_LowDM_Normalization_noZMassCut_noZPtCut  = makeStackMC_DiLepton_Normalization(  "passElecZinvNoZPtCut"  + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, ElectronWeights);
            std::vector<std::vector<PDS>> StackMC_Electron_HighDM_Normalization_noZMassCut_noZPtCut = makeStackMC_DiLepton_Normalization(  "passElecZinvNoZPtCut"  + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, ElectronWeights);
            // without Z mass cut; with Z pt cut
            ...
            // met
            PDC dcData_Electron_Baseline_met(           "data",   "metWithLL", {dsData_Electron_Baseline});
            PDC dcData_Electron_Baseline_met_nb0(       "data",   "metWithLL", {dsData_Electron_Baseline_nb0});
            PDC dcData_Electron_Baseline_met_nb1(       "data",   "metWithLL", {dsData_Electron_Baseline_nb1});
            PDC dcData_Electron_Baseline_met_nbge2(     "data",   "metWithLL", {dsData_Electron_Baseline_nbge2});
            PDC dcMC_Electron_Baseline_met(             "stack",  "metWithLL", StackMC_Electron_Baseline);
            ...
            PDC dcData_Electron_HighDM_nj( "data",   "nJets" + varSuffix, {dsData_Electron_HighDM});
            PDC dcMC_Electron_LowDM_nj(    "stack",  "nJets" + varSuffix, StackMC_Electron_LowDM);
            PDC dcMC_Electron_HighDM_nj(   "stack",  "nJets" + varSuffix, StackMC_Electron_HighDM);
            
            // n_jets_Loose
            PDC dcData_Electron_LowDM_Loose_nj(  "data",   "nJets" + varSuffix, {dsData_Electron_LowDM_Loose});
            PDC dcData_Electron_HighDM_Loose_nj( "data",   "nJets" + varSuffix, {dsData_Electron_HighDM_Loose});
            PDC dcMC_Electron_LowDM_Loose_nj(    "stack",  "nJets" + varSuffix, StackMC_Electron_LowDM_Loose);
            PDC dcMC_Electron_HighDM_Loose_nj(   "stack",  "nJets" + varSuffix, StackMC_Electron_HighDM_Loose);
            
            // n_jets_Mid
            PDC dcData_Electron_LowDM_Mid_nj(  "data",   "nJets" + varSuffix, {dsData_Electron_LowDM_Mid});
            PDC dcData_Electron_HighDM_Mid_nj( "data",   "nJets" + varSuffix, {dsData_Electron_HighDM_Mid});
            PDC dcMC_Electron_LowDM_Mid_nj(    "stack",  "nJets" + varSuffix, StackMC_Electron_LowDM_Mid);
            PDC dcMC_Electron_HighDM_Mid_nj(   "stack",  "nJets" + varSuffix, StackMC_Electron_HighDM_Mid);
            
            // HT
            PDC dcData_Electron_LowDM_ht(  "data",   "HT" + varSuffix, {dsData_Electron_LowDM});
            PDC dcData_Electron_HighDM_ht( "data",   "HT" + varSuffix, {dsData_Electron_HighDM});
            PDC dcMC_Electron_LowDM_ht(    "stack",  "HT" + varSuffix, StackMC_Electron_LowDM);
            PDC dcMC_Electron_HighDM_ht(   "stack",  "HT" + varSuffix, StackMC_Electron_HighDM);

            // HT_Loose
            PDC dcData_Electron_LowDM_Loose_ht(  "data",   "HT" + varSuffix, {dsData_Electron_LowDM_Loose});
            PDC dcData_Electron_HighDM_Loose_ht( "data",   "HT" + varSuffix, {dsData_Electron_HighDM_Loose});
            PDC dcMC_Electron_LowDM_Loose_ht(    "stack",  "HT" + varSuffix, StackMC_Electron_LowDM_Loose);
            PDC dcMC_Electron_HighDM_Loose_ht(   "stack",  "HT" + varSuffix, StackMC_Electron_HighDM_Loose);

            // HT_Mid
            PDC dcData_Electron_LowDM_Mid_ht(  "data",   "HT" + varSuffix, {dsData_Electron_LowDM_Mid});
            PDC dcData_Electron_HighDM_Mid_ht( "data",   "HT" + varSuffix, {dsData_Electron_HighDM_Mid});
            PDC dcMC_Electron_LowDM_Mid_ht(    "stack",  "HT" + varSuffix, StackMC_Electron_LowDM_Mid);
            PDC dcMC_Electron_HighDM_Mid_ht(   "stack",  "HT" + varSuffix, StackMC_Electron_HighDM_Mid);
            ...
            // with njetWeight
            // n_jets
            PDC dcData_Electron_LowDM_njetWeight_nj(  "data",   "nJets" + varSuffix, {dsData_Electron_LowDM});
            PDC dcData_Electron_HighDM_njetWeight_nj( "data",   "nJets" + varSuffix, {dsData_Electron_HighDM});
            PDC dcMC_Electron_LowDM_njetWeight_nj(    "stack",  "nJets" + varSuffix, StackMC_Electron_LowDM_njetWeight);
            PDC dcMC_Electron_HighDM_njetWeight_nj(   "stack",  "nJets" + varSuffix, StackMC_Electron_HighDM_njetWeight);
            
            // n_jets_Loose
            PDC dcData_Electron_LowDM_Loose_njetWeight_nj(  "data",   "nJets" + varSuffix, {dsData_Electron_LowDM_Loose});
            PDC dcData_Electron_HighDM_Loose_njetWeight_nj( "data",   "nJets" + varSuffix, {dsData_Electron_HighDM_Loose});
            PDC dcMC_Electron_LowDM_Loose_njetWeight_nj(    "stack",  "nJets" + varSuffix, StackMC_Electron_LowDM_Loose_njetWeight);
            PDC dcMC_Electron_HighDM_Loose_njetWeight_nj(   "stack",  "nJets" + varSuffix, StackMC_Electron_HighDM_Loose_njetWeight);
            
            // n_jets_Mid
            PDC dcData_Electron_LowDM_Mid_njetWeight_nj(  "data",   "nJets" + varSuffix, {dsData_Electron_LowDM_Mid});
            PDC dcData_Electron_HighDM_Mid_njetWeight_nj( "data",   "nJets" + varSuffix, {dsData_Electron_HighDM_Mid});
            PDC dcMC_Electron_LowDM_Mid_njetWeight_nj(    "stack",  "nJets" + varSuffix, StackMC_Electron_LowDM_Mid_njetWeight);
            PDC dcMC_Electron_HighDM_Mid_njetWeight_nj(   "stack",  "nJets" + varSuffix, StackMC_Electron_HighDM_Mid_njetWeight);
            
            // HT
            PDC dcData_Electron_LowDM_njetWeight_ht(  "data",   "HT" + varSuffix, {dsData_Electron_LowDM});
            PDC dcData_Electron_HighDM_njetWeight_ht( "data",   "HT" + varSuffix, {dsData_Electron_HighDM});
            PDC dcMC_Electron_LowDM_njetWeight_ht(    "stack",  "HT" + varSuffix, StackMC_Electron_LowDM_njetWeight);
            PDC dcMC_Electron_HighDM_njetWeight_ht(   "stack",  "HT" + varSuffix, StackMC_Electron_HighDM_njetWeight);

            // HT_Loose
            PDC dcData_Electron_LowDM_Loose_njetWeight_ht(  "data",   "HT" + varSuffix, {dsData_Electron_LowDM_Loose});
            PDC dcData_Electron_HighDM_Loose_njetWeight_ht( "data",   "HT" + varSuffix, {dsData_Electron_HighDM_Loose});
            PDC dcMC_Electron_LowDM_Loose_njetWeight_ht(    "stack",  "HT" + varSuffix, StackMC_Electron_LowDM_Loose_njetWeight);
            PDC dcMC_Electron_HighDM_Loose_njetWeight_ht(   "stack",  "HT" + varSuffix, StackMC_Electron_HighDM_Loose_njetWeight);

            // HT_Mid
            PDC dcData_Electron_LowDM_Mid_njetWeight_ht(  "data",   "HT" + varSuffix, {dsData_Electron_LowDM_Mid});
            PDC dcData_Electron_HighDM_Mid_njetWeight_ht( "data",   "HT" + varSuffix, {dsData_Electron_HighDM_Mid});
            PDC dcMC_Electron_LowDM_Mid_njetWeight_ht(    "stack",  "HT" + varSuffix, StackMC_Electron_LowDM_Mid_njetWeight);
            PDC dcMC_Electron_HighDM_Mid_njetWeight_ht(   "stack",  "HT" + varSuffix, StackMC_Electron_HighDM_Mid_njetWeight);

            // met
            PDC dcData_Electron_LowDM_njetWeight_met(  "data",   "metWithLL", {dsData_Electron_LowDM});
            PDC dcData_Electron_HighDM_njetWeight_met( "data",   "metWithLL", {dsData_Electron_HighDM});
            PDC dcMC_Electron_LowDM_njetWeight_met(    "stack",  "metWithLL", StackMC_Electron_LowDM_njetWeight);
            PDC dcMC_Electron_HighDM_njetWeight_met(   "stack",  "metWithLL", StackMC_Electron_HighDM_njetWeight);
            
            // met_Loose
            PDC dcData_Electron_LowDM_Loose_njetWeight_met(  "data",   "metWithLL", {dsData_Electron_LowDM_Loose});
            PDC dcData_Electron_HighDM_Loose_njetWeight_met( "data",   "metWithLL", {dsData_Electron_HighDM_Loose});
            ...
            // met_Mid
            PDC dcData_Electron_LowDM_Mid_njetWeight_met(  "data",   "metWithLL", {dsData_Electron_LowDM_Mid});
            PDC dcData_Electron_HighDM_Mid_njetWeight_met( "data",   "metWithLL", {dsData_Electron_HighDM_Mid});
            PDC dcMC_Electron_LowDM_Mid_njetWeight_met(    "stack",  "metWithLL", StackMC_Electron_LowDM_Mid_njetWeight);
            PDC dcMC_Electron_HighDM_Mid_njetWeight_met(   "stack",  "metWithLL", StackMC_Electron_HighDM_Mid_njetWeight);
            ...
            // bestRecoZPt: no Z mass or Pt cuts
            PDC dcData_Electron_LowDM_bestRecoZPt_noZMassCut_noZPtCut(  "data",   "bestRecoZPt", {dsData_Electron_LowDM_noZMassCut_noZPtCut});
            PDC dcData_Electron_HighDM_bestRecoZPt_noZMassCut_noZPtCut( "data",   "bestRecoZPt", {dsData_Electron_HighDM_noZMassCut_noZPtCut});
            PDC dcMC_Electron_LowDM_bestRecoZPt_noZMassCut_noZPtCut(    "stack",  "bestRecoZPt", StackMC_Electron_LowDM_noZMassCut_noZPtCut);
            PDC dcMC_Electron_HighDM_bestRecoZPt_noZMassCut_noZPtCut(   "stack",  "bestRecoZPt", StackMC_Electron_HighDM_noZMassCut_noZPtCut);
            
            // bestRecoZPt: without Z mass cut; with Z pt cut
            PDC dcData_Electron_LowDM_bestRecoZPt_noZMassCut(  "data",   "bestRecoZPt", {dsData_Electron_LowDM_noZMassCut});
            PDC dcData_Electron_HighDM_bestRecoZPt_noZMassCut( "data",   "bestRecoZPt", {dsData_Electron_HighDM_noZMassCut});
            PDC dcMC_Electron_LowDM_bestRecoZPt_noZMassCut(    "stack",  "bestRecoZPt", StackMC_Electron_LowDM_noZMassCut);
            ...
            PDC dcData_Electron_HighDM_bestRecoZPt_ZMassAndPtCuts( "data",   "bestRecoZPt", {dsData_Electron_HighDM_ZMassAndPtCuts});
            PDC dcMC_Electron_LowDM_bestRecoZPt_ZMassAndPtCuts(    "stack",  "bestRecoZPt", StackMC_Electron_LowDM_ZMassAndPtCuts);
            PDC dcMC_Electron_HighDM_bestRecoZPt_ZMassAndPtCuts(   "stack",  "bestRecoZPt", StackMC_Electron_HighDM_ZMassAndPtCuts);
            
            // bestRecoZM: without Z pt cut
            PDC dcData_Electron_LowDM_bestRecoZM_noZPtCut(  "data",   "bestRecoZM", {dsData_Electron_LowDM_noZMassCut_noZPtCut});
            PDC dcData_Electron_HighDM_bestRecoZM_noZPtCut( "data",   "bestRecoZM", {dsData_Electron_HighDM_noZMassCut_noZPtCut});
            PDC dcMC_Electron_LowDM_bestRecoZM_noZPtCut(    "stack",  "bestRecoZM", StackMC_Electron_LowDM_noZMassCut_noZPtCut);
            PDC dcMC_Electron_HighDM_bestRecoZM_noZPtCut(   "stack",  "bestRecoZM", StackMC_Electron_HighDM_noZMassCut_noZPtCut);
            PDC dcMC_Electron_LowDM_Normalization_bestRecoZM_noZPtCut(    "stack",  "bestRecoZM", StackMC_Electron_LowDM_Normalization_noZMassCut_noZPtCut);
            PDC dcMC_Electron_HighDM_Normalization_bestRecoZM_noZPtCut(   "stack",  "bestRecoZM", StackMC_Electron_HighDM_Normalization_noZMassCut_noZPtCut);
            
            // bestRecoZM: with Z pt cut
            PDC dcData_Electron_LowDM_bestRecoZM(  "data",   "bestRecoZM", {dsData_Electron_LowDM_noZMassCut});
            PDC dcData_Electron_HighDM_bestRecoZM( "data",   "bestRecoZM", {dsData_Electron_HighDM_noZMassCut});
            PDC dcMC_Electron_LowDM_bestRecoZM(    "stack",  "bestRecoZM", StackMC_Electron_LowDM_noZMassCut);
            PDC dcMC_Electron_HighDM_bestRecoZM(   "stack",  "bestRecoZM", StackMC_Electron_HighDM_noZMassCut);
            ...
            // mtb
            PDC dcData_Electron_LowDM_mtb(  "data",   "mtb" + varSuffix, {dsData_Electron_LowDM});
            PDC dcData_Electron_HighDM_mtb( "data",   "mtb" + varSuffix, {dsData_Electron_HighDM});
            PDC dcMC_Electron_LowDM_mtb(    "stack",  "mtb" + varSuffix, StackMC_Electron_LowDM);
            PDC dcMC_Electron_HighDM_mtb(   "stack",  "mtb" + varSuffix, StackMC_Electron_HighDM);
            
            // ptb
            PDC dcData_Electron_LowDM_ptb(  "data",   "ptb" + varSuffix, {dsData_Electron_LowDM});
            PDC dcData_Electron_HighDM_ptb( "data",   "ptb" + varSuffix, {dsData_Electron_HighDM});
            PDC dcMC_Electron_LowDM_ptb(    "stack",  "ptb" + varSuffix, StackMC_Electron_LowDM);
            PDC dcMC_Electron_HighDM_ptb(   "stack",  "ptb" + varSuffix, StackMC_Electron_HighDM);
            
            // ISRJetPt
            PDC dcData_Electron_LowDM_ISRJetPt(  "data",   "ISRJetPt" + JetPtCut, {dsData_Electron_LowDM});
            PDC dcData_Electron_HighDM_ISRJetPt( "data",   "ISRJetPt" + JetPtCut, {dsData_Electron_HighDM});
            PDC dcMC_Electron_LowDM_ISRJetPt(    "stack",  "ISRJetPt" + JetPtCut, StackMC_Electron_LowDM);
            PDC dcMC_Electron_HighDM_ISRJetPt(   "stack",  "ISRJetPt" + JetPtCut, StackMC_Electron_HighDM);
            
            // ISRJetPt_drLeptonCleaned
            PDC dcData_Electron_LowDM_ISRJetPt_drLeptonCleaned(  "data",   "ISRJetPt" + varSuffix, {dsData_Electron_LowDM});
            PDC dcData_Electron_HighDM_ISRJetPt_drLeptonCleaned( "data",   "ISRJetPt" + varSuffix, {dsData_Electron_HighDM});
            PDC dcMC_Electron_LowDM_ISRJetPt_drLeptonCleaned(    "stack",  "ISRJetPt" + varSuffix, StackMC_Electron_LowDM);
            PDC dcMC_Electron_HighDM_ISRJetPt_drLeptonCleaned(   "stack",  "ISRJetPt" + varSuffix, StackMC_Electron_HighDM);
            ...
            for (int i = 0; i < 4; i++)
            {
                std::string var = "dPhiVec" + varSuffix + "[" + std::to_string(i) + "]";
            ...
            // --- Study MET for different Nb --- //
            vh.push_back(PHS("DataMC_Electron_Baseline_met" + histSuffix,                                       {dcData_Electron_Baseline_met,                               dcMC_Electron_Baseline_met},                               {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithLL, "Events"));
            ...
            vh.push_back(PHS("DataMC_Electron_LowDM_ht" + histSuffix,                                           {dcData_Electron_LowDM_ht,                                   dcMC_Electron_LowDM_ht},                                   {1, 2}, "", nBins,  minHT, maxHT,                   true, doNorm, label_ht, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_ht" + histSuffix,                                          {dcData_Electron_HighDM_ht,                                  dcMC_Electron_HighDM_ht},                                  {1, 2}, "", nBins,  minHT, maxHT,                   true, doNorm, label_ht, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_met" + histSuffix,                                          {dcData_Electron_LowDM_met,                                  dcMC_Electron_LowDM_met},                                  {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_metWithLL, "Events"));
            ...
            vh.push_back(PHS("DataMC_Electron_HighDM_ElecPt2" + histSuffix,                                     {dcData_Electron_HighDM_ElecPt2,                             dcMC_Electron_HighDM_ElecPt2},                             {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_ElecPt2, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_ElecEta1" + histSuffix,                                     {dcData_Electron_LowDM_ElecEta1,                             dcMC_Electron_LowDM_ElecEta1},                             {1, 2}, "", nBins,  minEta, maxEta,                 true, doNorm, label_ElecEta1, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_ElecEta1" + histSuffix,                                    {dcData_Electron_HighDM_ElecEta1,                            dcMC_Electron_HighDM_ElecEta1},                            {1, 2}, "", nBins,  minEta, maxEta,                 true, doNorm, label_ElecEta1, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_ElecEta2" + histSuffix,                                     {dcData_Electron_LowDM_ElecEta2,                             dcMC_Electron_LowDM_ElecEta2},                             {1, 2}, "", nBins,  minEta, maxEta,                 true, doNorm, label_ElecEta2, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_ElecEta2" + histSuffix,                                    {dcData_Electron_HighDM_ElecEta2,                            dcMC_Electron_HighDM_ElecEta2},                            {1, 2}, "", nBins,  minEta, maxEta,                 true, doNorm, label_ElecEta2, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_bestRecoZPt_noZMassCut_noZPtCut" + histSuffix,              {dcData_Electron_LowDM_bestRecoZPt_noZMassCut_noZPtCut,      dcMC_Electron_LowDM_bestRecoZPt_noZMassCut_noZPtCut},      {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_bestRecoZPt, "Events"));
            ...
            vh.push_back(PHS("DataMC_Electron_LowDM_bestRecoZM_noZPtCut_50to250" + histSuffix,                  {dcData_Electron_LowDM_bestRecoZM_noZPtCut,                  dcMC_Electron_LowDM_bestRecoZM_noZPtCut},                  {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_bestRecoZM_noZPtCut_50to250" + histSuffix,                 {dcData_Electron_HighDM_bestRecoZM_noZPtCut,                 dcMC_Electron_HighDM_bestRecoZM_noZPtCut},                 {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_bestRecoZM_noZPtCut_0to400" + histSuffix,                   {dcData_Electron_LowDM_bestRecoZM_noZPtCut,                  dcMC_Electron_LowDM_bestRecoZM_noZPtCut},                  {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_bestRecoZM_noZPtCut_0to400" + histSuffix,                  {dcData_Electron_HighDM_bestRecoZM_noZPtCut,                 dcMC_Electron_HighDM_bestRecoZM_noZPtCut},                 {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_Normalization_bestRecoZM_noZPtCut_50to250" + histSuffix,    {dcData_Electron_LowDM_bestRecoZM_noZPtCut,                  dcMC_Electron_LowDM_Normalization_bestRecoZM_noZPtCut},    {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_Normalization_bestRecoZM_noZPtCut_50to250" + histSuffix,   {dcData_Electron_HighDM_bestRecoZM_noZPtCut,                 dcMC_Electron_HighDM_Normalization_bestRecoZM_noZPtCut},   {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_Normalization_bestRecoZM_noZPtCut_0to400" + histSuffix,     {dcData_Electron_LowDM_bestRecoZM_noZPtCut,                  dcMC_Electron_LowDM_Normalization_bestRecoZM_noZPtCut},    {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_Normalization_bestRecoZM_noZPtCut_0to400" + histSuffix,    {dcData_Electron_HighDM_bestRecoZM_noZPtCut,                 dcMC_Electron_HighDM_Normalization_bestRecoZM_noZPtCut},   {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_bestRecoZM_50to250" + histSuffix,                           {dcData_Electron_LowDM_bestRecoZM,                           dcMC_Electron_LowDM_bestRecoZM},                           {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_bestRecoZM_50to250" + histSuffix,                          {dcData_Electron_HighDM_bestRecoZM,                          dcMC_Electron_HighDM_bestRecoZM},                          {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_bestRecoZM_0to400" + histSuffix,                            {dcData_Electron_LowDM_bestRecoZM,                           dcMC_Electron_LowDM_bestRecoZM},                           {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_bestRecoZM_0to400" + histSuffix,                           {dcData_Electron_HighDM_bestRecoZM,                          dcMC_Electron_HighDM_bestRecoZM},                          {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_Normalization_bestRecoZM_50to250" + histSuffix,             {dcData_Electron_LowDM_bestRecoZM,                           dcMC_Electron_LowDM_Normalization_bestRecoZM},             {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_Normalization_bestRecoZM_50to250" + histSuffix,            {dcData_Electron_HighDM_bestRecoZM,                          dcMC_Electron_HighDM_Normalization_bestRecoZM},            {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_Normalization_bestRecoZM_0to400" + histSuffix,              {dcData_Electron_LowDM_bestRecoZM,                           dcMC_Electron_LowDM_Normalization_bestRecoZM},             {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_Normalization_bestRecoZM_0to400" + histSuffix,             {dcData_Electron_HighDM_bestRecoZM,                          dcMC_Electron_HighDM_Normalization_bestRecoZM},            {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_mtb" + histSuffix,                                          {dcData_Electron_LowDM_mtb,                                  dcMC_Electron_LowDM_mtb},                                  {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_mtb, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_mtb" + histSuffix,                                         {dcData_Electron_HighDM_mtb,                                 dcMC_Electron_HighDM_mtb},                                 {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_mtb, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_ptb" + histSuffix,                                          {dcData_Electron_LowDM_ptb,                                  dcMC_Electron_LowDM_ptb},                                  {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_ptb, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_ptb" + histSuffix,                                         {dcData_Electron_HighDM_ptb,                                 dcMC_Electron_HighDM_ptb},                                 {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_ptb, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_ISRJetPt" + histSuffix,                                     {dcData_Electron_LowDM_ISRJetPt,                             dcMC_Electron_LowDM_ISRJetPt},                             {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_ISRJetPt, "Events"));
            vh.push_back(PHS("DataMC_Electron_HighDM_ISRJetPt" + histSuffix,                                    {dcData_Electron_HighDM_ISRJetPt,                            dcMC_Electron_HighDM_ISRJetPt},                            {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_ISRJetPt, "Events"));
            vh.push_back(PHS("DataMC_Electron_LowDM_ISRJetPt_drLeptonCleaned" + histSuffix,                     {dcData_Electron_LowDM_ISRJetPt_drLeptonCleaned,             dcMC_Electron_LowDM_ISRJetPt_drLeptonCleaned},             {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, "lepton cleaned "+label_ISRJetPt, "Events"));
            ...
            if (doLooseAndMid)
            {
                ...
                vh.push_back(PHS("DataMC_Electron_LowDM_Loose_ht" + histSuffix,                            {dcData_Electron_LowDM_Loose_ht,                     dcMC_Electron_LowDM_Loose_ht},                     {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                vh.push_back(PHS("DataMC_Electron_HighDM_Loose_ht" + histSuffix,                           {dcData_Electron_HighDM_Loose_ht,                    dcMC_Electron_HighDM_Loose_ht},                    {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                ...
                vh.push_back(PHS("DataMC_Electron_LowDM_Mid_ht" + histSuffix,                              {dcData_Electron_LowDM_Mid_ht,                       dcMC_Electron_LowDM_Mid_ht},                       {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                vh.push_back(PHS("DataMC_Electron_HighDM_Mid_ht" + histSuffix,                             {dcData_Electron_HighDM_Mid_ht,                      dcMC_Electron_HighDM_Mid_ht},                      {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                ...
                vh.push_back(PHS("DataMC_Electron_LowDM_njetWeight_ht" + histSuffix,                                  {dcData_Electron_LowDM_njetWeight_ht,                           dcMC_Electron_LowDM_njetWeight_ht},                           {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                vh.push_back(PHS("DataMC_Electron_HighDM_njetWeight_ht" + histSuffix,                                 {dcData_Electron_HighDM_njetWeight_ht,                          dcMC_Electron_HighDM_njetWeight_ht},                          {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                ...
                vh.push_back(PHS("DataMC_Electron_LowDM_Loose_njetWeight_ht" + histSuffix,                            {dcData_Electron_LowDM_Loose_njetWeight_ht,                     dcMC_Electron_LowDM_Loose_njetWeight_ht},                     {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                vh.push_back(PHS("DataMC_Electron_HighDM_Loose_njetWeight_ht" + histSuffix,                           {dcData_Electron_HighDM_Loose_njetWeight_ht,                    dcMC_Electron_HighDM_Loose_njetWeight_ht},                    {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                ...
                vh.push_back(PHS("DataMC_Electron_LowDM_Mid_njetWeight_ht" + histSuffix,                              {dcData_Electron_LowDM_Mid_njetWeight_ht,                       dcMC_Electron_LowDM_Mid_njetWeight_ht},                       {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                vh.push_back(PHS("DataMC_Electron_HighDM_Mid_njetWeight_ht" + histSuffix,                             {dcData_Electron_HighDM_Mid_njetWeight_ht,                      dcMC_Electron_HighDM_Mid_njetWeight_ht},                      {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
            ...
            for (int i = 0; i < 4; i++)
            {
                std::string nameLowDM  = "DataMC_Electron_LowDM_dPhi"  + std::to_string(i+1) + histSuffix;
                std::string nameHighDM = "DataMC_Electron_HighDM_dPhi" + std::to_string(i+1) + histSuffix;
                vh.push_back(PHS(nameLowDM,  {dcVecData_Electron_LowDM_dPhi[i], dcVecMC_Electron_LowDM_dPhi[i]},    {1, 2}, "", nBins,  0.0, maxPhi, true, doNorm, vec_label_dphi[i], "Events"));
                vh.push_back(PHS(nameHighDM, {dcVecData_Electron_HighDM_dPhi[i], dcVecMC_Electron_HighDM_dPhi[i]},  {1, 2}, "", nBins,  0.0, maxPhi, true, doNorm, vec_label_dphi[i], "Events"));
            }
            ...
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_Data_Electron_LowDM_met"  + histSuffix,   dcData_Electron_LowDM_met,  CutLevels_Data_Electron_LowDM));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_Data_Electron_HighDM_met" + histSuffix,   dcData_Electron_HighDM_met, CutLevels_Data_Electron_HighDM));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_MC_Electron_LowDM_met"    + histSuffix,   dcMC_Electron_LowDM_met,    CutLevels_MC_Electron_LowDM));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_MC_Electron_HighDM_met"   + histSuffix,   dcMC_Electron_HighDM_met,   CutLevels_MC_Electron_HighDM));
        }
        ...
        if (doDataMCMuon)
        {
            ...
            for (const auto& cut : map_norm_cuts_low_dm)
            {
                ...
                std::vector<std::vector<PDS>> StackMC_Muon_LowDM_Normalization_noZMassCut_split = makeStackMC_DiLepton_Normalization(  "passMuZinvSel"      + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned + cut.second, MuonWeights, true);
                // bestRecoZM
                ...
                PDC dcMC_Muon_LowDM_Normalization_bestRecoZM_split( "stack",  "bestRecoZM", StackMC_Muon_LowDM_Normalization_noZMassCut_split);
                vh.push_back(PHS("DataMC_Muon_LowDM_bestRecoZM_50to250_" + cut.first + histSuffix,                      {dcData_Muon_LowDM_bestRecoZM,   dcMC_Muon_LowDM_bestRecoZM},                       {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Muon_LowDM_bestRecoZM_0to400_" + cut.first + histSuffix,                       {dcData_Muon_LowDM_bestRecoZM,   dcMC_Muon_LowDM_bestRecoZM},                       {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Muon_LowDM_Normalization_bestRecoZM_50to250_" + cut.first + histSuffix,        {dcData_Muon_LowDM_bestRecoZM,   dcMC_Muon_LowDM_Normalization_bestRecoZM},         {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Muon_LowDM_Normalization_bestRecoZM_0to400_" + cut.first + histSuffix,         {dcData_Muon_LowDM_bestRecoZM,   dcMC_Muon_LowDM_Normalization_bestRecoZM},         {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Muon_LowDM_Normalization_bestRecoZM_split_50to250_" + cut.first + histSuffix,  {dcData_Muon_LowDM_bestRecoZM,   dcMC_Muon_LowDM_Normalization_bestRecoZM_split},   {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Muon_LowDM_Normalization_bestRecoZM_split_0to400_" + cut.first + histSuffix,   {dcData_Muon_LowDM_bestRecoZM,   dcMC_Muon_LowDM_Normalization_bestRecoZM_split},   {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
            ...
            for (const auto& cut : map_norm_cuts_high_dm)
            {
                ...
                std::vector<std::vector<PDS>> StackMC_Muon_HighDM_Normalization_noZMassCut_split = makeStackMC_DiLepton_Normalization(  "passMuZinvSel"   + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned + cut.second, MuonWeights, true);
                // bestRecoZM
                ...
                PDC dcMC_Muon_HighDM_Normalization_bestRecoZM_split( "stack",  "bestRecoZM", StackMC_Muon_HighDM_Normalization_noZMassCut_split);
                vh.push_back(PHS("DataMC_Muon_HighDM_bestRecoZM_50to250_" + cut.first + histSuffix,                     {dcData_Muon_HighDM_bestRecoZM,   dcMC_Muon_HighDM_bestRecoZM},                     {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Muon_HighDM_bestRecoZM_0to400_" + cut.first + histSuffix,                      {dcData_Muon_HighDM_bestRecoZM,   dcMC_Muon_HighDM_bestRecoZM},                     {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Muon_HighDM_Normalization_bestRecoZM_50to250_" + cut.first + histSuffix,       {dcData_Muon_HighDM_bestRecoZM,   dcMC_Muon_HighDM_Normalization_bestRecoZM},       {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Muon_HighDM_Normalization_bestRecoZM_0to400_" + cut.first + histSuffix,        {dcData_Muon_HighDM_bestRecoZM,   dcMC_Muon_HighDM_Normalization_bestRecoZM},       {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Muon_HighDM_Normalization_bestRecoZM_split_50to250_" + cut.first + histSuffix, {dcData_Muon_HighDM_bestRecoZM,   dcMC_Muon_HighDM_Normalization_bestRecoZM_split}, {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
                vh.push_back(PHS("DataMC_Muon_HighDM_Normalization_bestRecoZM_split_0to400_" + cut.first + histSuffix,  {dcData_Muon_HighDM_bestRecoZM,   dcMC_Muon_HighDM_Normalization_bestRecoZM_split}, {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY, true, false, label_bestRecoZM, "Events"));
            ...
            // Note: Apply Flag_eeBadScFilter to Data but not MC
            // no Z mass or Pt cuts
            PDS dsData_Muon_LowDM_noZMassCut_noZPtCut("Data",   fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvNoZPtCut;Pass_trigger_muon" + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_HighDM_noZMassCut_noZPtCut("Data",  fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvNoZPtCut;Pass_trigger_muon" + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            // without Z mass cut; with Z pt cut
            ...
            // with on Z mass peak cut; without Z pt cut
            PDS dsData_Muon_LowDM_noZPtCut("Data",              fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;Pass_trigger_muon"  + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_HighDM_noZPtCut("Data",             fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;Pass_trigger_muon"  + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            // with Z mass and Pt cuts
            PDS dsData_Muon_LowDM_ZMassAndPtCuts("Data",        fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon" + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_HighDM_ZMassAndPtCuts("Data",       fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon" + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            // with Z mass and Pt cuts
            PDS dsData_Muon_Baseline("Data",        fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon"  + SAT_Pass_Baseline + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_Baseline_nb0("Data",    fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon;nBottoms_drLeptonCleaned_jetpt30=0"  + SAT_Pass_Baseline + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_Baseline_nb1("Data",    fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon;nBottoms_drLeptonCleaned_jetpt30=1"  + SAT_Pass_Baseline + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_Baseline_nbge2("Data",  fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon;nBottoms_drLeptonCleaned_jetpt30>=2" + SAT_Pass_Baseline + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_LowDM("Data",           fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon"  + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_HighDM("Data",          fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon"  + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_LowDM_Loose("Data",     fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon"  + SAT_Pass_lowDM_Loose + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_HighDM_Loose("Data",    fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon"  + SAT_Pass_highDM_Loose + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_LowDM_Mid("Data",       fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon"  + SAT_Pass_lowDM_Mid + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            PDS dsData_Muon_HighDM_Mid("Data",      fileMap[MuonDataset], "Flag_eeBadScFilter;passMuZinvOnZMassPeak;passDiMuPt;Pass_trigger_muon"  + SAT_Pass_highDM_Mid + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, "");
            
            // MC
            // no Z mass or Pt cuts
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM_noZMassCut_noZPtCut                 = makeStackMC_DiLepton(                 "passMuZinvNoZPtCut" + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM_noZMassCut_noZPtCut                = makeStackMC_DiLepton(                 "passMuZinvNoZPtCut" + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM_Normalization_noZMassCut_noZPtCut   = makeStackMC_DiLepton_Normalization(   "passMuZinvNoZPtCut" + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM_Normalization_noZMassCut_noZPtCut  = makeStackMC_DiLepton_Normalization(   "passMuZinvNoZPtCut" + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            // without Z mass cut; with Z pt cut
            ...
            // with on Z mass peak cut; without Z pt cut
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM_noZPtCut        = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak" + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights); 
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM_noZPtCut       = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak" + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            // with Z mass and Pt cuts
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM_ZMassAndPtCuts  = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights); 
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM_ZMassAndPtCuts = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            // with Z mass and Pt cuts
            std::vector<std::vector<PDS>> StackMC_Muon_Baseline              = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_Baseline + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_Baseline_nb0          = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt;nBottoms_drLeptonCleaned_jetpt30=0"  + SAT_Pass_Baseline + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_Baseline_nb1          = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt;nBottoms_drLeptonCleaned_jetpt30=1"  + SAT_Pass_Baseline + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_Baseline_nbge2        = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt;nBottoms_drLeptonCleaned_jetpt30>=2" + SAT_Pass_Baseline + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM                 = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights); 
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM                = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM_Loose           = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_lowDM_Loose + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM_Loose          = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_highDM_Loose + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM_Mid             = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_lowDM_Mid + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM_Mid            = makeStackMC_DiLepton(                 "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_highDM_Mid + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM_Normalization   = makeStackMC_DiLepton_Normalization(   "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM_Normalization  = makeStackMC_DiLepton_Normalization(   "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights);

            // with shape weight
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM_njetWeight        = makeStackMC_DiLepton(               "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_lowDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights, ";njetWeight_Muon_LowDM");
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM_njetWeight       = makeStackMC_DiLepton(               "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_highDM + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights, ";njetWeight_Muon_HighDM");
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM_Loose_njetWeight  = makeStackMC_DiLepton(               "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_lowDM_Loose + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights, ";njetWeight_Muon_LowDM");
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM_Loose_njetWeight = makeStackMC_DiLepton(               "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_highDM_Loose + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights, ";njetWeight_Muon_HighDM");
            std::vector<std::vector<PDS>> StackMC_Muon_LowDM_Mid_njetWeight    = makeStackMC_DiLepton(               "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_lowDM_Mid + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights, ";njetWeight_Muon_LowDM");
            std::vector<std::vector<PDS>> StackMC_Muon_HighDM_Mid_njetWeight   = makeStackMC_DiLepton(               "passMuZinvOnZMassPeak;passDiMuPt" + SAT_Pass_highDM_Mid + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drLeptonCleaned, MuonWeights, ";njetWeight_Muon_HighDM");
            
            // --- Study MET for different Nb --- //
            // met
            PDC dcData_Muon_Baseline_met(           "data",   "metWithLL", {dsData_Muon_Baseline});
            PDC dcData_Muon_Baseline_met_nb0(       "data",   "metWithLL", {dsData_Muon_Baseline_nb0});
            PDC dcData_Muon_Baseline_met_nb1(       "data",   "metWithLL", {dsData_Muon_Baseline_nb1});
            PDC dcData_Muon_Baseline_met_nbge2(     "data",   "metWithLL", {dsData_Muon_Baseline_nbge2});
            PDC dcMC_Muon_Baseline_met(             "stack",  "metWithLL", StackMC_Muon_Baseline);
            PDC dcMC_Muon_Baseline_met_nb0(         "stack",  "metWithLL", StackMC_Muon_Baseline_nb0);
            ...
            PDC dcData_Muon_HighDM_nj( "data",   "nJets" + varSuffix, {dsData_Muon_HighDM});
            PDC dcMC_Muon_LowDM_nj(    "stack",  "nJets" + varSuffix, StackMC_Muon_LowDM);
            PDC dcMC_Muon_HighDM_nj(   "stack",  "nJets" + varSuffix, StackMC_Muon_HighDM);
            
            // n_jets_Loose
            PDC dcData_Muon_LowDM_Loose_nj(  "data",   "nJets" + varSuffix, {dsData_Muon_LowDM_Loose});
            PDC dcData_Muon_HighDM_Loose_nj( "data",   "nJets" + varSuffix, {dsData_Muon_HighDM_Loose});
            PDC dcMC_Muon_LowDM_Loose_nj(    "stack",  "nJets" + varSuffix, StackMC_Muon_LowDM_Loose);
            PDC dcMC_Muon_HighDM_Loose_nj(   "stack",  "nJets" + varSuffix, StackMC_Muon_HighDM_Loose);

            // n_jets_Mid
            PDC dcData_Muon_LowDM_Mid_nj(  "data",   "nJets" + varSuffix, {dsData_Muon_LowDM_Mid});
            PDC dcData_Muon_HighDM_Mid_nj( "data",   "nJets" + varSuffix, {dsData_Muon_HighDM_Mid});
            PDC dcMC_Muon_LowDM_Mid_nj(    "stack",  "nJets" + varSuffix, StackMC_Muon_LowDM_Mid);
            PDC dcMC_Muon_HighDM_Mid_nj(   "stack",  "nJets" + varSuffix, StackMC_Muon_HighDM_Mid);

            // HT
            PDC dcData_Muon_LowDM_ht(  "data",   "HT" + varSuffix, {dsData_Muon_LowDM});
            PDC dcData_Muon_HighDM_ht( "data",   "HT" + varSuffix, {dsData_Muon_HighDM});
            PDC dcMC_Muon_LowDM_ht(    "stack",  "HT" + varSuffix, StackMC_Muon_LowDM);
            PDC dcMC_Muon_HighDM_ht(   "stack",  "HT" + varSuffix, StackMC_Muon_HighDM);

            // HT_Loose
            PDC dcData_Muon_LowDM_Loose_ht(  "data",   "HT" + varSuffix, {dsData_Muon_LowDM_Loose});
            PDC dcData_Muon_HighDM_Loose_ht( "data",   "HT" + varSuffix, {dsData_Muon_HighDM_Loose});
            PDC dcMC_Muon_LowDM_Loose_ht(    "stack",  "HT" + varSuffix, StackMC_Muon_LowDM_Loose);
            PDC dcMC_Muon_HighDM_Loose_ht(   "stack",  "HT" + varSuffix, StackMC_Muon_HighDM_Loose);

            // HT_Mid
            PDC dcData_Muon_LowDM_Mid_ht(  "data",   "HT" + varSuffix, {dsData_Muon_LowDM_Mid});
            PDC dcData_Muon_HighDM_Mid_ht( "data",   "HT" + varSuffix, {dsData_Muon_HighDM_Mid});
            PDC dcMC_Muon_LowDM_Mid_ht(    "stack",  "HT" + varSuffix, StackMC_Muon_LowDM_Mid);
            PDC dcMC_Muon_HighDM_Mid_ht(   "stack",  "HT" + varSuffix, StackMC_Muon_HighDM_Mid);
            ...
            //With shape weight
            // n_jets
            PDC dcData_Muon_LowDM_njetWeight_nj(  "data",   "nJets" + varSuffix, {dsData_Muon_LowDM});
            PDC dcData_Muon_HighDM_njetWeight_nj( "data",   "nJets" + varSuffix, {dsData_Muon_HighDM});
            PDC dcMC_Muon_LowDM_njetWeight_nj(    "stack",  "nJets" + varSuffix, StackMC_Muon_LowDM_njetWeight);
            PDC dcMC_Muon_HighDM_njetWeight_nj(   "stack",  "nJets" + varSuffix, StackMC_Muon_HighDM_njetWeight);
            
            // n_jets_Loose
            PDC dcData_Muon_LowDM_Loose_njetWeight_nj(  "data",   "nJets" + varSuffix, {dsData_Muon_LowDM_Loose});
            PDC dcData_Muon_HighDM_Loose_njetWeight_nj( "data",   "nJets" + varSuffix, {dsData_Muon_HighDM_Loose});
            PDC dcMC_Muon_LowDM_Loose_njetWeight_nj(    "stack",  "nJets" + varSuffix, StackMC_Muon_LowDM_Loose_njetWeight);
            PDC dcMC_Muon_HighDM_Loose_njetWeight_nj(   "stack",  "nJets" + varSuffix, StackMC_Muon_HighDM_Loose_njetWeight);

            // n_jets_Mid
            PDC dcData_Muon_LowDM_Mid_njetWeight_nj(  "data",   "nJets" + varSuffix, {dsData_Muon_LowDM_Mid});
            PDC dcData_Muon_HighDM_Mid_njetWeight_nj( "data",   "nJets" + varSuffix, {dsData_Muon_HighDM_Mid});
            PDC dcMC_Muon_LowDM_Mid_njetWeight_nj(    "stack",  "nJets" + varSuffix, StackMC_Muon_LowDM_Mid_njetWeight);
            PDC dcMC_Muon_HighDM_Mid_njetWeight_nj(   "stack",  "nJets" + varSuffix, StackMC_Muon_HighDM_Mid_njetWeight);

            // HT
            PDC dcData_Muon_LowDM_njetWeight_ht(  "data",   "HT" + varSuffix, {dsData_Muon_LowDM});
            PDC dcData_Muon_HighDM_njetWeight_ht( "data",   "HT" + varSuffix, {dsData_Muon_HighDM});
            PDC dcMC_Muon_LowDM_njetWeight_ht(    "stack",  "HT" + varSuffix, StackMC_Muon_LowDM_njetWeight);
            PDC dcMC_Muon_HighDM_njetWeight_ht(   "stack",  "HT" + varSuffix, StackMC_Muon_HighDM_njetWeight);

            // HT_Loose
            PDC dcData_Muon_LowDM_Loose_njetWeight_ht(  "data",   "HT" + varSuffix, {dsData_Muon_LowDM_Loose});
            PDC dcData_Muon_HighDM_Loose_njetWeight_ht( "data",   "HT" + varSuffix, {dsData_Muon_HighDM_Loose});
            PDC dcMC_Muon_LowDM_Loose_njetWeight_ht(    "stack",  "HT" + varSuffix, StackMC_Muon_LowDM_Loose_njetWeight);
            PDC dcMC_Muon_HighDM_Loose_njetWeight_ht(   "stack",  "HT" + varSuffix, StackMC_Muon_HighDM_Loose_njetWeight);

            // HT_Mid
            PDC dcData_Muon_LowDM_Mid_njetWeight_ht(  "data",   "HT" + varSuffix, {dsData_Muon_LowDM_Mid});
            PDC dcData_Muon_HighDM_Mid_njetWeight_ht( "data",   "HT" + varSuffix, {dsData_Muon_HighDM_Mid});
            PDC dcMC_Muon_LowDM_Mid_njetWeight_ht(    "stack",  "HT" + varSuffix, StackMC_Muon_LowDM_Mid_njetWeight);
            PDC dcMC_Muon_HighDM_Mid_njetWeight_ht(   "stack",  "HT" + varSuffix, StackMC_Muon_HighDM_Mid_njetWeight);

            // met
            PDC dcData_Muon_LowDM_njetWeight_met(  "data",   "metWithLL", {dsData_Muon_LowDM});
            PDC dcData_Muon_HighDM_njetWeight_met( "data",   "metWithLL", {dsData_Muon_HighDM});
            PDC dcMC_Muon_LowDM_njetWeight_met(    "stack",  "metWithLL", StackMC_Muon_LowDM_njetWeight);
            PDC dcMC_Muon_HighDM_njetWeight_met(   "stack",  "metWithLL", StackMC_Muon_HighDM_njetWeight);

            // met_Loose
            PDC dcData_Muon_LowDM_Loose_njetWeight_met(  "data",   "metWithLL", {dsData_Muon_LowDM_Loose});
            ...
            // met_Mid
            PDC dcData_Muon_LowDM_Mid_njetWeight_met(  "data",   "metWithLL", {dsData_Muon_LowDM_Mid});
            PDC dcData_Muon_HighDM_Mid_njetWeight_met( "data",   "metWithLL", {dsData_Muon_HighDM_Mid});
            PDC dcMC_Muon_LowDM_Mid_njetWeight_met(    "stack",  "metWithLL", StackMC_Muon_LowDM_Mid_njetWeight);
            PDC dcMC_Muon_HighDM_Mid_njetWeight_met(   "stack",  "metWithLL", StackMC_Muon_HighDM_Mid_njetWeight);
            ...
            // bestRecoZPt: no Z mass or Pt cuts
            PDC dcData_Muon_LowDM_bestRecoZPt_noZMassCut_noZPtCut(  "data",   "bestRecoZPt", {dsData_Muon_LowDM_noZMassCut_noZPtCut});
            PDC dcData_Muon_HighDM_bestRecoZPt_noZMassCut_noZPtCut( "data",   "bestRecoZPt", {dsData_Muon_HighDM_noZMassCut_noZPtCut});
            PDC dcMC_Muon_LowDM_bestRecoZPt_noZMassCut_noZPtCut(    "stack",  "bestRecoZPt", StackMC_Muon_LowDM_noZMassCut_noZPtCut);
            PDC dcMC_Muon_HighDM_bestRecoZPt_noZMassCut_noZPtCut(   "stack",  "bestRecoZPt", StackMC_Muon_HighDM_noZMassCut_noZPtCut);
            
            // bestRecoZPt: without Z mass cut; with Z pt cut
            PDC dcData_Muon_LowDM_bestRecoZPt_noZMassCut(  "data",   "bestRecoZPt", {dsData_Muon_LowDM_noZMassCut});
            PDC dcData_Muon_HighDM_bestRecoZPt_noZMassCut( "data",   "bestRecoZPt", {dsData_Muon_HighDM_noZMassCut});
            PDC dcMC_Muon_LowDM_bestRecoZPt_noZMassCut(    "stack",  "bestRecoZPt", StackMC_Muon_LowDM_noZMassCut);
            PDC dcMC_Muon_HighDM_bestRecoZPt_noZMassCut(   "stack",  "bestRecoZPt", StackMC_Muon_HighDM_noZMassCut);
            
            // bestRecoZPt: with on Z mass peak cut; without Z pt cut
            PDC dcData_Muon_LowDM_bestRecoZPt_noZPtCut(  "data",   "bestRecoZPt", {dsData_Muon_LowDM_noZPtCut});
            PDC dcData_Muon_HighDM_bestRecoZPt_noZPtCut( "data",   "bestRecoZPt", {dsData_Muon_HighDM_noZPtCut});
            PDC dcMC_Muon_LowDM_bestRecoZPt_noZPtCut(    "stack",  "bestRecoZPt", StackMC_Muon_LowDM_noZPtCut);
            PDC dcMC_Muon_HighDM_bestRecoZPt_noZPtCut(   "stack",  "bestRecoZPt", StackMC_Muon_HighDM_noZPtCut);
            
            // bestRecoZPt: with Z mass and Pt cuts 
            PDC dcData_Muon_LowDM_bestRecoZPt_ZMassAndPtCuts(  "data",   "bestRecoZPt", {dsData_Muon_LowDM_ZMassAndPtCuts});
            PDC dcData_Muon_HighDM_bestRecoZPt_ZMassAndPtCuts( "data",   "bestRecoZPt", {dsData_Muon_HighDM_ZMassAndPtCuts});
            PDC dcMC_Muon_LowDM_bestRecoZPt_ZMassAndPtCuts(    "stack",  "bestRecoZPt", StackMC_Muon_LowDM_ZMassAndPtCuts);
            PDC dcMC_Muon_HighDM_bestRecoZPt_ZMassAndPtCuts(   "stack",  "bestRecoZPt", StackMC_Muon_HighDM_ZMassAndPtCuts);
            
            // bestRecoZM: without Z pt cut
            PDC dcData_Muon_LowDM_bestRecoZM_noZPtCut(  "data",   "bestRecoZM", {dsData_Muon_LowDM_noZMassCut_noZPtCut});
            PDC dcData_Muon_HighDM_bestRecoZM_noZPtCut( "data",   "bestRecoZM", {dsData_Muon_HighDM_noZMassCut_noZPtCut});
            PDC dcMC_Muon_LowDM_bestRecoZM_noZPtCut(    "stack",  "bestRecoZM", StackMC_Muon_LowDM_noZMassCut_noZPtCut);
            PDC dcMC_Muon_HighDM_bestRecoZM_noZPtCut(   "stack",  "bestRecoZM", StackMC_Muon_HighDM_noZMassCut_noZPtCut);
            PDC dcMC_Muon_LowDM_Normalization_bestRecoZM_noZPtCut(    "stack",  "bestRecoZM", StackMC_Muon_LowDM_Normalization_noZMassCut_noZPtCut);
            PDC dcMC_Muon_HighDM_Normalization_bestRecoZM_noZPtCut(   "stack",  "bestRecoZM", StackMC_Muon_HighDM_Normalization_noZMassCut_noZPtCut);
            ...
            // mtb
            PDC dcData_Muon_LowDM_mtb(  "data",   "mtb" + varSuffix, {dsData_Muon_LowDM});
            PDC dcData_Muon_HighDM_mtb( "data",   "mtb" + varSuffix, {dsData_Muon_HighDM});
            PDC dcMC_Muon_LowDM_mtb(    "stack",  "mtb" + varSuffix, StackMC_Muon_LowDM);
            PDC dcMC_Muon_HighDM_mtb(   "stack",  "mtb" + varSuffix, StackMC_Muon_HighDM);
            
            // ptb
            PDC dcData_Muon_LowDM_ptb(  "data",   "ptb" + varSuffix, {dsData_Muon_LowDM});
            PDC dcData_Muon_HighDM_ptb( "data",   "ptb" + varSuffix, {dsData_Muon_HighDM});
            PDC dcMC_Muon_LowDM_ptb(    "stack",  "ptb" + varSuffix, StackMC_Muon_LowDM);
            PDC dcMC_Muon_HighDM_ptb(   "stack",  "ptb" + varSuffix, StackMC_Muon_HighDM);
            ...
            // --- Study MET for different Nb --- //
            vh.push_back(PHS("DataMC_Muon_Baseline_met" + histSuffix,                                       {dcData_Muon_Baseline_met,                              dcMC_Muon_Baseline_met},                                {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_metWithLL, "Events"));
            ...
            vh.push_back(PHS("DataMC_Muon_LowDM_ht" + histSuffix,                                           {dcData_Muon_LowDM_ht,                                  dcMC_Muon_LowDM_ht},                                    {1, 2}, "",   nBins,  minHT, maxHT,                 true, doNorm, label_ht, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_ht" + histSuffix,                                          {dcData_Muon_HighDM_ht,                                 dcMC_Muon_HighDM_ht},                                   {1, 2}, "",   nBins,  minHT, maxHT,                 true, doNorm, label_ht, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_met" + histSuffix,                                          {dcData_Muon_LowDM_met,                                 dcMC_Muon_LowDM_met},                                   {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_metWithLL, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_met" + histSuffix,                                         {dcData_Muon_HighDM_met,                                dcMC_Muon_HighDM_met},                                  {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_metWithLL, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_metphi" + histSuffix,                                       {dcData_Muon_LowDM_metphi,                              dcMC_Muon_LowDM_metphi},                                {1, 2}, "", nBins,  minPhi, maxPhi,                 true, doNorm, label_metphiWithLL, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_metphi" + histSuffix,                                      {dcData_Muon_HighDM_metphi,                             dcMC_Muon_HighDM_metphi},                               {1, 2}, "", nBins,  minPhi, maxPhi,                 true, doNorm, label_metphiWithLL, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_MuPt1" + histSuffix,                                        {dcData_Muon_LowDM_MuPt1,                               dcMC_Muon_LowDM_MuPt1},                                 {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_MuPt1, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_MuPt1" + histSuffix,                                       {dcData_Muon_HighDM_MuPt1,                              dcMC_Muon_HighDM_MuPt1},                                {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_MuPt1, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_MuPt2" + histSuffix,                                        {dcData_Muon_LowDM_MuPt2,                               dcMC_Muon_LowDM_MuPt2},                                 {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_MuPt2, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_MuPt2" + histSuffix,                                       {dcData_Muon_HighDM_MuPt2,                              dcMC_Muon_HighDM_MuPt2},                                {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_MuPt2, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_MuEta1" + histSuffix,                                       {dcData_Muon_LowDM_MuEta1,                              dcMC_Muon_LowDM_MuEta1},                                {1, 2}, "", nBins,  minEta, maxEta,                 true, doNorm, label_MuEta1, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_MuEta1" + histSuffix,                                      {dcData_Muon_HighDM_MuEta1,                             dcMC_Muon_HighDM_MuEta1},                               {1, 2}, "", nBins,  minEta, maxEta,                 true, doNorm, label_MuEta1, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_MuEta2" + histSuffix,                                       {dcData_Muon_LowDM_MuEta2,                              dcMC_Muon_LowDM_MuEta2},                                {1, 2}, "", nBins,  minEta, maxEta,                 true, doNorm, label_MuEta2, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_MuEta2" + histSuffix,                                      {dcData_Muon_HighDM_MuEta2,                             dcMC_Muon_HighDM_MuEta2},                               {1, 2}, "", nBins,  minEta, maxEta,                 true, doNorm, label_MuEta2, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_bestRecoZPt_noZMassCut_noZPtCut" + histSuffix,              {dcData_Muon_LowDM_bestRecoZPt_noZMassCut_noZPtCut,     dcMC_Muon_LowDM_bestRecoZPt_noZMassCut_noZPtCut},       {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_bestRecoZPt, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_bestRecoZPt_noZMassCut_noZPtCut" + histSuffix,             {dcData_Muon_HighDM_bestRecoZPt_noZMassCut_noZPtCut,    dcMC_Muon_HighDM_bestRecoZPt_noZMassCut_noZPtCut},      {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_bestRecoZPt, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_bestRecoZPt_noZMassCut" + histSuffix,                       {dcData_Muon_LowDM_bestRecoZPt_noZMassCut,              dcMC_Muon_LowDM_bestRecoZPt_noZMassCut},                {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_bestRecoZPt, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_bestRecoZPt_noZMassCut" + histSuffix,                      {dcData_Muon_HighDM_bestRecoZPt_noZMassCut,             dcMC_Muon_HighDM_bestRecoZPt_noZMassCut},               {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_bestRecoZPt, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_bestRecoZPt_noZPtCut" + histSuffix,                         {dcData_Muon_LowDM_bestRecoZPt_noZPtCut,                dcMC_Muon_LowDM_bestRecoZPt_noZPtCut},                  {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_bestRecoZPt, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_bestRecoZPt_noZPtCut" + histSuffix,                        {dcData_Muon_HighDM_bestRecoZPt_noZPtCut,               dcMC_Muon_HighDM_bestRecoZPt_noZPtCut},                 {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_bestRecoZPt, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_bestRecoZPt_ZMassAndPtCuts" + histSuffix,                   {dcData_Muon_LowDM_bestRecoZPt_ZMassAndPtCuts,          dcMC_Muon_LowDM_bestRecoZPt_ZMassAndPtCuts},            {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_bestRecoZPt, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_bestRecoZPt_ZMassAndPtCuts" + histSuffix,                  {dcData_Muon_HighDM_bestRecoZPt_ZMassAndPtCuts,         dcMC_Muon_HighDM_bestRecoZPt_ZMassAndPtCuts},           {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_bestRecoZPt, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_bestRecoZM_noZPtCut_50to250" + histSuffix,                  {dcData_Muon_LowDM_bestRecoZM_noZPtCut,                 dcMC_Muon_LowDM_bestRecoZM_noZPtCut},                   {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_bestRecoZM_noZPtCut_50to250" + histSuffix,                 {dcData_Muon_HighDM_bestRecoZM_noZPtCut,                dcMC_Muon_HighDM_bestRecoZM_noZPtCut},                  {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_bestRecoZM_noZPtCut_0to400" + histSuffix,                   {dcData_Muon_LowDM_bestRecoZM_noZPtCut,                 dcMC_Muon_LowDM_bestRecoZM_noZPtCut},                   {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_bestRecoZM_noZPtCut_0to400" + histSuffix,                  {dcData_Muon_HighDM_bestRecoZM_noZPtCut,                dcMC_Muon_HighDM_bestRecoZM_noZPtCut},                  {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_Normalization_bestRecoZM_noZPtCut_50to250" + histSuffix,    {dcData_Muon_LowDM_bestRecoZM_noZPtCut,                 dcMC_Muon_LowDM_Normalization_bestRecoZM_noZPtCut},     {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_Normalization_bestRecoZM_noZPtCut_50to250" + histSuffix,   {dcData_Muon_HighDM_bestRecoZM_noZPtCut,                dcMC_Muon_HighDM_Normalization_bestRecoZM_noZPtCut},    {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_Normalization_bestRecoZM_noZPtCut_0to400" + histSuffix,     {dcData_Muon_LowDM_bestRecoZM_noZPtCut,                 dcMC_Muon_LowDM_Normalization_bestRecoZM_noZPtCut},     {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_Normalization_bestRecoZM_noZPtCut_0to400" + histSuffix,    {dcData_Muon_HighDM_bestRecoZM_noZPtCut,                dcMC_Muon_HighDM_Normalization_bestRecoZM_noZPtCut},    {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_bestRecoZM_50to250" + histSuffix,                           {dcData_Muon_LowDM_bestRecoZM,                          dcMC_Muon_LowDM_bestRecoZM},                            {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_bestRecoZM_50to250" + histSuffix,                          {dcData_Muon_HighDM_bestRecoZM,                         dcMC_Muon_HighDM_bestRecoZM},                           {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_bestRecoZM_0to400" + histSuffix,                            {dcData_Muon_LowDM_bestRecoZM,                          dcMC_Muon_LowDM_bestRecoZM},                            {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_bestRecoZM_0to400" + histSuffix,                           {dcData_Muon_HighDM_bestRecoZM,                         dcMC_Muon_HighDM_bestRecoZM},                           {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_Normalization_bestRecoZM_50to250" + histSuffix,             {dcData_Muon_LowDM_bestRecoZM,                          dcMC_Muon_LowDM_Normalization_bestRecoZM},              {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_Normalization_bestRecoZM_50to250" + histSuffix,            {dcData_Muon_HighDM_bestRecoZM,                         dcMC_Muon_HighDM_Normalization_bestRecoZM},             {1, 2}, "", 40, 50.0, 250.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_Normalization_bestRecoZM_0to400" + histSuffix,              {dcData_Muon_LowDM_bestRecoZM,                          dcMC_Muon_LowDM_Normalization_bestRecoZM},              {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_Normalization_bestRecoZM_0to400" + histSuffix,             {dcData_Muon_HighDM_bestRecoZM,                         dcMC_Muon_HighDM_Normalization_bestRecoZM},             {1, 2}, "", 400, 0.0, 400.0, massMinY, massMaxY,    true, doNorm, label_bestRecoZM, "Events"));
            vh.push_back(PHS("DataMC_Muon_LowDM_mtb" + histSuffix,                                          {dcData_Muon_LowDM_mtb,                                 dcMC_Muon_LowDM_mtb},                                   {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_mtb, "Events"));
            vh.push_back(PHS("DataMC_Muon_HighDM_mtb" + histSuffix,                                         {dcData_Muon_HighDM_mtb,                                dcMC_Muon_HighDM_mtb},                                  {1, 2}, "", nBins,  minPt, maxPt,                   true, doNorm, label_mtb, "Events"));
            ...
            if (doLooseAndMid)
            {
                ...
                vh.push_back(PHS("DataMC_Muon_LowDM_Loose_ht" + histSuffix,                            {dcData_Muon_LowDM_Loose_ht,     dcMC_Muon_LowDM_Loose_ht},                  {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                vh.push_back(PHS("DataMC_Muon_HighDM_Loose_ht" + histSuffix,                           {dcData_Muon_HighDM_Loose_ht,    dcMC_Muon_HighDM_Loose_ht},                 {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                ...
                vh.push_back(PHS("DataMC_Muon_LowDM_Mid_ht" + histSuffix,                              {dcData_Muon_LowDM_Mid_ht,       dcMC_Muon_LowDM_Mid_ht},                    {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                vh.push_back(PHS("DataMC_Muon_HighDM_Mid_ht" + histSuffix,                             {dcData_Muon_HighDM_Mid_ht,      dcMC_Muon_HighDM_Mid_ht},                   {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                ...
                vh.push_back(PHS("DataMC_Muon_LowDM_njetWeight_ht" + histSuffix,                                  {dcData_Muon_LowDM_njetWeight_ht,           dcMC_Muon_LowDM_njetWeight_ht},                        {1, 2}, "",   nBins,  minHT, maxHT,      true, doNorm, label_ht, "Events"));
                vh.push_back(PHS("DataMC_Muon_HighDM_njetWeight_ht" + histSuffix,                                 {dcData_Muon_HighDM_njetWeight_ht,          dcMC_Muon_HighDM_njetWeight_ht},                       {1, 2}, "",   nBins,  minHT, maxHT,      true, doNorm, label_ht, "Events"));
                ...
                vh.push_back(PHS("DataMC_Muon_LowDM_Loose_njetWeight_ht" + histSuffix,                            {dcData_Muon_LowDM_Loose_njetWeight_ht,     dcMC_Muon_LowDM_Loose_njetWeight_ht},                  {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                vh.push_back(PHS("DataMC_Muon_HighDM_Loose_njetWeight_ht" + histSuffix,                           {dcData_Muon_HighDM_Loose_njetWeight_ht,    dcMC_Muon_HighDM_Loose_njetWeight_ht},                 {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                ...
                vh.push_back(PHS("DataMC_Muon_LowDM_Mid_njetWeight_ht" + histSuffix,                              {dcData_Muon_LowDM_Mid_njetWeight_ht,       dcMC_Muon_LowDM_Mid_njetWeight_ht},                    {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
                vh.push_back(PHS("DataMC_Muon_HighDM_Mid_njetWeight_ht" + histSuffix,                             {dcData_Muon_HighDM_Mid_njetWeight_ht,      dcMC_Muon_HighDM_Mid_njetWeight_ht},                   {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, "Events"));
            ...
            for (int i = 0; i < 4; i++)
            {
                std::string nameLowDM  = "DataMC_Muon_LowDM_dPhi"  + std::to_string(i+1) + histSuffix;
                std::string nameHighDM = "DataMC_Muon_HighDM_dPhi" + std::to_string(i+1) + histSuffix;
                vh.push_back(PHS(nameLowDM,  {dcVecData_Muon_LowDM_dPhi[i], dcVecMC_Muon_LowDM_dPhi[i]},    {1, 2}, "", nBins,  0.0, maxPhi, true, doNorm, vec_label_dphi[i], "Events"));
                vh.push_back(PHS(nameHighDM, {dcVecData_Muon_HighDM_dPhi[i], dcVecMC_Muon_HighDM_dPhi[i]},  {1, 2}, "", nBins,  0.0, maxPhi, true, doNorm, vec_label_dphi[i], "Events"));
            }
            
            // cut flow: name, DataCollection, cutLevels
            // Plotter::CutFlowSummary::CutFlowSummary(std::string n, Plotter::DataCollection ns, std::vector<std::string> cutLevels)
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_Data_Muon_LowDM_met"  + histSuffix, dcData_Muon_LowDM_met,  CutLevels_Data_Muon_LowDM));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_Data_Muon_HighDM_met" + histSuffix, dcData_Muon_HighDM_met, CutLevels_Data_Muon_HighDM));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_MC_Muon_LowDM_met"    + histSuffix, dcMC_Muon_LowDM_met,    CutLevels_MC_Muon_LowDM));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_MC_Muon_HighDM_met"   + histSuffix, dcMC_Muon_HighDM_met,   CutLevels_MC_Muon_HighDM));
        }
        ...
        std::map<std::string, std::string> photon_var_map = {
            {"NB",  "nBottoms"     + varSuffix},
            {"NSV", "nSoftBottoms" + varSuffix},
            {"NJ",  "nJets"        + varSuffix},
        };
        std::vector<std::string> vec_shape_cuts_low_dm  = {"NBeq0_NJle5", "NBeq0_NJge6", "NBeq0", "NBeq1", "NBge1", "NBge2", "NBge2_NJge7"};
        std::vector<std::string> vec_shape_cuts_high_dm = {"NBeq1", "NBeq1_NJge7", "NBeq2", "NBge2", "NBge2_NJge7", "NBge3"};
        std::map<std::string, std::string> map_shape_cuts_low_dm;
        std::map<std::string, std::string> map_shape_cuts_high_dm;
        for (auto& cut : vec_shape_cuts_low_dm)
        {
            map_shape_cuts_low_dm[cut] = ";" + SusyUtility::parseCuts(cut, photon_var_map);
        }
        for (auto& cut : vec_shape_cuts_high_dm)
        {
        ...
        if (doDataMCPhoton)
        {
            ...
            for (const auto& cut : map_shape_cuts_low_dm)
            {
                ...
                if (doSystematics)
                {
                    ...
                    for (const auto& jec : jesMap)
                    {
                        std::string histSuffixSyst = "_jes_syst_" + jec.first + JetPtCut;
                        std::string SAT_Pass_lowDM_jec = SAT_Pass_lowDM + jec.second;
                        std::vector<std::vector<PDS>> StackMC_Photon_LowDM_jec          = makeStackMC_Photon( "MET_pt<250"                          + PhotonIDSelection + SAT_Pass_lowDM_jec         + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drPhotonCleaned + cut.second, PhotonWeights);
                        PDC dcMC_Photon_LowDM_met_jec(                               "stack",  "metWithPhoton", StackMC_Photon_LowDM_jec);
                        vh.push_back(PHS("DataMC_Photon_LowDM_met_" + cut.first + histSuffixSyst,                              {dcData_Photon_LowDM_met,                              dcMC_Photon_LowDM_met_jec},                              {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, "Events"));
                    }
                }
            }
            // --------------- //
            ...
            for (const auto& cut : map_shape_cuts_high_dm)
            {
                ...
                if (doSystematics)
                {
                    ...
                    for (const auto& jec : jesMap)
                    {
                        std::string histSuffixSyst = "_jes_syst_" + jec.first + JetPtCut;
                        std::string SAT_Pass_highDM_jec = SAT_Pass_highDM + jec.second;
                        std::vector<std::vector<PDS>> StackMC_Photon_HighDM_jec          = makeStackMC_Photon( "MET_pt<250"                          + PhotonIDSelection + SAT_Pass_highDM_jec         + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drPhotonCleaned + cut.second, PhotonWeights);
                        PDC dcMC_Photon_HighDM_met_jec(                               "stack",  "metWithPhoton", StackMC_Photon_HighDM_jec);
                        vh.push_back(PHS("DataMC_Photon_HighDM_met_" + cut.first + histSuffixSyst,                              {dcData_Photon_HighDM_met,                              dcMC_Photon_HighDM_met_jec},                              {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, "Events"));
                    }
                }
            }
            // ------------------- //
            ...
            if (doSystematics)
            {
                ...
                for (const auto& pdf : pdfMap)
                {
                    ...
                    vh.push_back(PHS("DataMC_Photon_LowDM_nCRUnitLowDM" + histSuffixSyst,                {dcData_Photon_LowDM_nCRUnitLowDM,                     dcMC_Photon_LowDM_nCRUnitLowDM},                     {1, 2}, "", max_crunit_low_dm - min_crunit_low_dm,    min_crunit_low_dm,  max_crunit_low_dm,   true, false, "Control Region Unit Low DM",  "Events"));
                    vh.push_back(PHS("DataMC_Photon_HighDM_nCRUnitHighDM" + histSuffixSyst,              {dcData_Photon_HighDM_nCRUnitHighDM,                   dcMC_Photon_HighDM_nCRUnitHighDM},                   {1, 2}, "", max_crunit_high_dm - min_crunit_high_dm,  min_crunit_high_dm, max_crunit_high_dm,  true, false, "Control Region Unit High DM", "Events"));
                }
                // --- JEC systematic --- //
                // only apply JEC to MC, not Data
                for (const auto& jec : jesMap)
                {
                    ...
                    std::string SAT_Pass_lowDM_jec  = SAT_Pass_lowDM  + jec.second;
                    std::string SAT_Pass_highDM_jec = SAT_Pass_highDM + jec.second;
                    std::vector<std::vector<PDS>> StackMC_Photon_LowDM   = makeStackMC_Photon( "MET_pt<250" + PhotonIDSelection + SAT_Pass_lowDM_jec  + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drPhotonCleaned, PhotonWeights);
                    std::vector<std::vector<PDS>> StackMC_Photon_HighDM  = makeStackMC_Photon( "MET_pt<250" + PhotonIDSelection + SAT_Pass_highDM_jec + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drPhotonCleaned, PhotonWeights);
                    // Control Region Units: nCRUnitLowDM and nCRUnitHighDM
                    PDC dcMC_Photon_LowDM_nCRUnitLowDM(         "stack",  "nCRUnitLowDM"  + varSuffix + jec.second,   StackMC_Photon_LowDM);
                    PDC dcMC_Photon_HighDM_nCRUnitHighDM(       "stack",  "nCRUnitHighDM" + varSuffix + jec.second,   StackMC_Photon_HighDM);
                    vh.push_back(PHS("DataMC_Photon_LowDM_nCRUnitLowDM" + histSuffixSyst,                {dcData_Photon_LowDM_nCRUnitLowDM,                     dcMC_Photon_LowDM_nCRUnitLowDM},                     {1, 2}, "", max_crunit_low_dm - min_crunit_low_dm,    min_crunit_low_dm,  max_crunit_low_dm,   true, false, "Control Region Unit Low DM",  "Events"));
                    vh.push_back(PHS("DataMC_Photon_HighDM_nCRUnitHighDM" + histSuffixSyst,              {dcData_Photon_HighDM_nCRUnitHighDM,                   dcMC_Photon_HighDM_nCRUnitHighDM},                   {1, 2}, "", max_crunit_high_dm - min_crunit_high_dm,  min_crunit_high_dm, max_crunit_high_dm,  true, false, "Control Region Unit High DM", "Events"));
                }
            ...
            // MC
            std::vector<std::vector<PDS>> StackMC_Photon_NoBaseline             = makeStackMC_Photon_split( "passPhotonSelection"                       + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drPhotonCleaned, PhotonWeights);
            std::vector<std::vector<PDS>> StackMC_Photon_NoBaseline_withQCDcut  = makeStackMC_Photon_split( "passPhotonSelection"                       + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drPhotonCleaned, PhotonWeights, ";passQCDSelection");
            ...
            std::vector<std::vector<PDS>> StackMC_Photon_LowDM_split            = makeStackMC_Photon_split( "MET_pt<250"                                + PhotonIDSelection + SAT_Pass_lowDM         + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drPhotonCleaned, PhotonWeights, ";passQCDSelection");
            ...
            std::vector<std::vector<PDS>> StackMC_Photon_HighDM_split           = makeStackMC_Photon_split( "MET_pt<250"                                + PhotonIDSelection + SAT_Pass_highDM        + Flag_ecalBadCalibFilter + semicolon_HEMVeto_drPhotonCleaned, PhotonWeights, ";passQCDSelection");
            ...
            // n_bottoms
            PDC dcData_Photon_LowDM_nb_nominal(  "data",   "nBottoms" + JetPtCut, {dsData_Photon_LowDM});
            PDC dcData_Photon_HighDM_nb_nominal( "data",   "nBottoms" + JetPtCut, {dsData_Photon_HighDM});
            ...
            PDC dcMC_Photon_LowDM_nmt_nominal(    "stack",  "nMergedTops" + JetPtCut, StackMC_Photon_LowDM);
            PDC dcMC_Photon_HighDM_nmt_nominal(   "stack",  "nMergedTops" + JetPtCut, StackMC_Photon_HighDM);
            
            // n_Ws
            PDC dcData_Photon_LowDM_nw_nominal(  "data",   "nWs" + JetPtCut, {dsData_Photon_LowDM});
            PDC dcData_Photon_HighDM_nw_nominal( "data",   "nWs" + JetPtCut, {dsData_Photon_HighDM});
            PDC dcMC_Photon_LowDM_nw_nominal(    "stack",  "nWs" + JetPtCut, StackMC_Photon_LowDM);
            PDC dcMC_Photon_HighDM_nw_nominal(   "stack",  "nWs" + JetPtCut, StackMC_Photon_HighDM);
            
            // n_resolvedTops
            PDC dcData_Photon_LowDM_nrt_nominal(  "data",   "nResolvedTops" + JetPtCut, {dsData_Photon_LowDM});
            PDC dcData_Photon_HighDM_nrt_nominal( "data",   "nResolvedTops" + JetPtCut, {dsData_Photon_HighDM});
            PDC dcMC_Photon_LowDM_nrt_nominal(    "stack",  "nResolvedTops" + JetPtCut, StackMC_Photon_LowDM);
            PDC dcMC_Photon_HighDM_nrt_nominal(   "stack",  "nResolvedTops" + JetPtCut, StackMC_Photon_HighDM);
            
            // n_jets
            PDC dcData_Photon_LowDM_nj_nominal(  "data",   "nJets" + JetPtCut, {dsData_Photon_LowDM});
            PDC dcData_Photon_HighDM_nj_nominal( "data",   "nJets" + JetPtCut, {dsData_Photon_HighDM});
            PDC dcMC_Photon_LowDM_nj_nominal(    "stack",  "nJets" + JetPtCut, StackMC_Photon_LowDM);
            PDC dcMC_Photon_HighDM_nj_nominal(   "stack",  "nJets" + JetPtCut, StackMC_Photon_HighDM);
            ...
            PDC dcData_Photon_LowDM_Tight_metphi(                    "data",   "metphiWithPhoton", {dsData_Photon_LowDM_Tight});
            ...
            PDC dcMC_Photon_LowDM_Tight_metphi(                      "stack",  "metphiWithPhoton", StackMC_Photon_LowDM_Tight);
            ...
            PDC dcData_Photon_LowDM_Tight_PhotonPt(                  "data",   "cutPhotonPt", {dsData_Photon_LowDM_Tight});
            ...
            PDC dcMC_Photon_LowDM_Tight_PhotonPt(                    "stack",  "cutPhotonPt", StackMC_Photon_LowDM_Tight);
            ...
            PDC dcData_Photon_LowDM_Tight_PhotonEta(                 "data",   "cutPhotonEta", {dsData_Photon_LowDM_Tight});
            ...
            PDC dcMC_Photon_LowDM_Tight_PhotonEta(                   "stack",  "cutPhotonEta", StackMC_Photon_LowDM_Tight);
            ...
            PDC dcData_Photon_LowDM_Tight_dR_GenPhotonGenParton(     "data",   "dR_GenPhotonGenParton", {dsData_Photon_LowDM_Tight});
            ...
            PDC dcMC_Photon_LowDM_Tight_dR_GenPhotonGenParton(       "stack",  "dR_GenPhotonGenParton", StackMC_Photon_LowDM_Tight);
            ...
            PDC dcMC_Photon_NoBaseline_withQCDcut_dR_PromptPhotonGenParton( "stack",  "dR_PromptPhotonGenParton", StackMC_Photon_NoBaseline_withQCDcut);
            ...
            PDC dcData_Photon_LowDM_Tight_dR_RecoPhotonGenPhoton(    "data",   "dR_RecoPhotonGenPhoton", {dsData_Photon_LowDM_Tight});
            ...
            PDC dcMC_Photon_LowDM_Tight_dR_RecoPhotonGenPhoton(      "stack",  "dR_RecoPhotonGenPhoton", StackMC_Photon_LowDM_Tight);
            ...
            PDC dcData_Photon_LowDM_Tight_min_dR_GenPhotonGenParton( "data",   "min_dR_GenPhotonGenParton", {dsData_Photon_LowDM_Tight});
            ...
            PDC dcMC_Photon_LowDM_Tight_min_dR_GenPhotonGenParton(   "stack",  "min_dR_GenPhotonGenParton", StackMC_Photon_LowDM_Tight);
            ...
            PDC dcMC_Photon_NoBaseline_withQCDcut_min_dR_PromptPhotonGenParton( "stack",  "min_dR_PromptPhotonGenParton", StackMC_Photon_NoBaseline_withQCDcut);
            ...
            PDC dcData_Photon_LowDM_Tight_min_dR_RecoPhotonGenPhoton( "data",   "min_dR_RecoPhotonGenPhoton", {dsData_Photon_LowDM_Tight});
            ...
            std::vector<PDC> dcVecMC_Photon_LowDM_Tight_dPhi;
            std::vector<                                     PDC> dcVecMC_Photon_HighDM_dPhi;
            for (int i = 0; i < 4; i++)
            {
                std::string var = "dPhiVec" + varSuffix + "[" + std::to_string(i) + "]";
                dcVecData_Photon_LowDM_dPhi.push_back(       PDC("data", var, {dsData_Photon_LowDM}));
                dcVecData_Photon_LowDM_Tight_dPhi.push_back( PDC("data", var, {dsData_Photon_LowDM_Tight}));
                ...
                dcVecMC_Photon_LowDM_Tight_dPhi.push_back(   PDC("stack", var, StackMC_Photon_LowDM_Tight));
                dcVecMC_Photon_HighDM_dPhi.push_back(        PDC("stack", var, StackMC_Photon_HighDM));
            }


            const bool doNorm = true;
            std::string label_y_axis = "Events";
            if (doNorm)
            {
                label_y_axis = "Events (Normalized)";
            }
            ...
            std::string CRUnitBins_53to56_lowmtb_nrtge1_Cuts    = "nBottoms_drPhotonCleaned_jetpt30=1;nJets_drPhotonCleaned_jetpt30>=7;SAT_Pass_MTB_LowDM_drPhotonCleaned_jetpt30;nResolvedTops_drPhotonCleaned_jetpt30>=1";
            std::string CRUnitBins_57to60_lowmtb_nrtge1_Cuts    = "nBottoms_drPhotonCleaned_jetpt30>=2;nJets_drPhotonCleaned_jetpt30>=7;SAT_Pass_MTB_LowDM_drPhotonCleaned_jetpt30;nResolvedTops_drPhotonCleaned_jetpt30>=1";
            std::string CRUnitBins_53to56_lowmtb_nmtge1_Cuts    = "nBottoms_drPhotonCleaned_jetpt30=1;nJets_drPhotonCleaned_jetpt30>=7;SAT_Pass_MTB_LowDM_drPhotonCleaned_jetpt30;nMergedTops_drPhotonCleaned_jetpt30>=1";
            std::string CRUnitBins_57to60_lowmtb_nmtge1_Cuts    = "nBottoms_drPhotonCleaned_jetpt30>=2;nJets_drPhotonCleaned_jetpt30>=7;SAT_Pass_MTB_LowDM_drPhotonCleaned_jetpt30;nMergedTops_drPhotonCleaned_jetpt30>=1";
            std::string CRUnitBins_53to56_lowmtb_nwge1_Cuts     = "nBottoms_drPhotonCleaned_jetpt30=1;nJets_drPhotonCleaned_jetpt30>=7;SAT_Pass_MTB_LowDM_drPhotonCleaned_jetpt30;nWs_drPhotonCleaned_jetpt30>=1";
            std::string CRUnitBins_57to60_lowmtb_nwge1_Cuts     = "nBottoms_drPhotonCleaned_jetpt30>=2;nJets_drPhotonCleaned_jetpt30>=7;SAT_Pass_MTB_LowDM_drPhotonCleaned_jetpt30;nWs_drPhotonCleaned_jetpt30>=1";
            
            // --- check CR unit bins 53-60 --- //
            vh.push_back(PHS("DataMC_Photon_CRUnitBins_53to56_basic" + histSuffix,                                  {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2},   CRUnitBins_53to56_basic_Cuts,           myCRUnitBinEdges,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_CRUnitBins_57to60_basic" + histSuffix,                                  {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2},   CRUnitBins_57to60_basic_Cuts,           myCRUnitBinEdges,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_CRUnitBins_53to56_lowmtb" + histSuffix,                                 {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2},   CRUnitBins_53to56_lowmtb_Cuts,          myCRUnitBinEdges,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_CRUnitBins_57to60_lowmtb" + histSuffix,                                 {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2},   CRUnitBins_57to60_lowmtb_Cuts,          myCRUnitBinEdges,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_CRUnitBins_53to56_lowmtb_nrtge1" + histSuffix,                          {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2},   CRUnitBins_53to56_lowmtb_nrtge1_Cuts,   myCRUnitBinEdges,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_CRUnitBins_57to60_lowmtb_nrtge1" + histSuffix,                          {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2},   CRUnitBins_57to60_lowmtb_nrtge1_Cuts,   myCRUnitBinEdges,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_CRUnitBins_53to56_lowmtb_nmtge1" + histSuffix,                          {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2},   CRUnitBins_53to56_lowmtb_nmtge1_Cuts,   myCRUnitBinEdges,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_CRUnitBins_57to60_lowmtb_nmtge1" + histSuffix,                          {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2},   CRUnitBins_57to60_lowmtb_nmtge1_Cuts,   myCRUnitBinEdges,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_CRUnitBins_53to56_lowmtb_nwge1" + histSuffix,                           {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2},   CRUnitBins_53to56_lowmtb_nwge1_Cuts,    myCRUnitBinEdges,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_CRUnitBins_57to60_lowmtb_nwge1" + histSuffix,                           {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2},   CRUnitBins_57to60_lowmtb_nwge1_Cuts,    myCRUnitBinEdges,        true, doNorm, label_metWithPhoton, label_y_axis));
            // --- Study MET for different Nb --- //
            vh.push_back(PHS("DataMC_Photon_Baseline_met" + histSuffix,                                             {dcData_Photon_Baseline_met,                                        dcMC_Photon_Baseline_met},                                          {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_Baseline_met_nb0" + histSuffix,                                         {dcData_Photon_Baseline_met_nb0,                                    dcMC_Photon_Baseline_met_nb0},                                      {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_Baseline_met_nb1" + histSuffix,                                         {dcData_Photon_Baseline_met_nb1,                                    dcMC_Photon_Baseline_met_nb1},                                      {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_Baseline_met_nbge2" + histSuffix,                                       {dcData_Photon_Baseline_met_nbge2,                                  dcMC_Photon_Baseline_met_nbge2},                                    {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, label_y_axis));
            // --- Study variables using baseline (not low/high dm) --- //
            vh.push_back(PHS("DataMC_Photon_Baseline_nb" + histSuffix,                                              {dcData_Photon_Baseline_nb,                                         dcMC_Photon_Baseline_nb},                                           {1, 2}, "", 6,  0,  6, true, doNorm, label_nb,  label_y_axis));
            vh.push_back(PHS("DataMC_Photon_Baseline_nsv" + histSuffix,                                             {dcData_Photon_Baseline_nsv,                                        dcMC_Photon_Baseline_nsv},                                          {1, 2}, "", 6,  0,  6, true, doNorm, label_nsv, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_Baseline_nj" + histSuffix,                                              {dcData_Photon_Baseline_nj,                                         dcMC_Photon_Baseline_nj},                                           {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            // --- Nominal: without jet cleaning --- //
            vh.push_back(PHS("DataMC_Photon_LowDM_nb_nominal" + histSuffix,                                         {dcData_Photon_LowDM_nb_nominal,                                    dcMC_Photon_LowDM_nb_nominal},                                      {1, 2}, "", 6,  0,  6, true, doNorm, label_nb,  label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nb_nominal" + histSuffix,                                        {dcData_Photon_HighDM_nb_nominal,                                   dcMC_Photon_HighDM_nb_nominal},                                     {1, 2}, "", 6,  0,  6, true, doNorm, label_nb,  label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nsv_nominal" + histSuffix,                                        {dcData_Photon_LowDM_nsv_nominal,                                   dcMC_Photon_LowDM_nsv_nominal},                                     {1, 2}, "", 6,  0,  6, true, doNorm, label_nsv, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nsv_nominal" + histSuffix,                                       {dcData_Photon_HighDM_nsv_nominal,                                  dcMC_Photon_HighDM_nsv_nominal},                                    {1, 2}, "", 6,  0,  6, true, doNorm, label_nsv, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nw_nominal" + histSuffix,                                         {dcData_Photon_LowDM_nw_nominal,                                    dcMC_Photon_LowDM_nw_nominal},                                      {1, 2}, "", 6,  0,  6, true, doNorm, label_nw,  label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nw_nominal" + histSuffix,                                        {dcData_Photon_HighDM_nw_nominal,                                   dcMC_Photon_HighDM_nw_nominal},                                     {1, 2}, "", 6,  0,  6, true, doNorm, label_nw,  label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nmt_nominal" + histSuffix,                                        {dcData_Photon_LowDM_nmt_nominal,                                   dcMC_Photon_LowDM_nmt_nominal},                                     {1, 2}, "", 6,  0,  6, true, doNorm, label_nmt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nmt_nominal" + histSuffix,                                       {dcData_Photon_HighDM_nmt_nominal,                                  dcMC_Photon_HighDM_nmt_nominal},                                    {1, 2}, "", 6,  0,  6, true, doNorm, label_nmt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nrt_nominal" + histSuffix,                                        {dcData_Photon_LowDM_nrt_nominal,                                   dcMC_Photon_LowDM_nrt_nominal},                                     {1, 2}, "", 6,  0,  6, true, doNorm, label_nrt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nrt_nominal" + histSuffix,                                       {dcData_Photon_HighDM_nrt_nominal,                                  dcMC_Photon_HighDM_nrt_nominal},                                    {1, 2}, "", 6,  0,  6, true, doNorm, label_nrt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nj_nominal" + histSuffix,                                         {dcData_Photon_LowDM_nj_nominal,                                    dcMC_Photon_LowDM_nj_nominal},                                      {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nj_nominal" + histSuffix,                                        {dcData_Photon_HighDM_nj_nominal,                                   dcMC_Photon_HighDM_nj_nominal},                                     {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            // --- Standard Selection --- //
            vh.push_back(PHS("DataMC_Photon_LowDM_nb" + histSuffix,                                                 {dcData_Photon_LowDM_nb,                                            dcMC_Photon_LowDM_nb},                                              {1, 2}, "", 6,  0,  6, true, doNorm, label_nb,  label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nb" + histSuffix,                                                {dcData_Photon_HighDM_nb,                                           dcMC_Photon_HighDM_nb},                                             {1, 2}, "", 6,  0,  6, true, doNorm, label_nb,  label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nsv" + histSuffix,                                                {dcData_Photon_LowDM_nsv,                                           dcMC_Photon_LowDM_nsv},                                             {1, 2}, "", 6,  0,  6, true, doNorm, label_nsv, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nsv" + histSuffix,                                               {dcData_Photon_HighDM_nsv,                                          dcMC_Photon_HighDM_nsv},                                            {1, 2}, "", 6,  0,  6, true, doNorm, label_nsv, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nw" + histSuffix,                                                 {dcData_Photon_LowDM_nw,                                            dcMC_Photon_LowDM_nw},                                              {1, 2}, "", 6,  0,  6, true, doNorm, label_nw,  label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nw" + histSuffix,                                                {dcData_Photon_HighDM_nw,                                           dcMC_Photon_HighDM_nw},                                             {1, 2}, "", 6,  0,  6, true, doNorm, label_nw,  label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nmt" + histSuffix,                                                {dcData_Photon_LowDM_nmt,                                           dcMC_Photon_LowDM_nmt},                                             {1, 2}, "", 6,  0,  6, true, doNorm, label_nmt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nmt" + histSuffix,                                               {dcData_Photon_HighDM_nmt,                                          dcMC_Photon_HighDM_nmt},                                            {1, 2}, "", 6,  0,  6, true, doNorm, label_nmt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nrt" + histSuffix,                                                {dcData_Photon_LowDM_nrt,                                           dcMC_Photon_LowDM_nrt},                                             {1, 2}, "", 6,  0,  6, true, doNorm, label_nrt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nrt" + histSuffix,                                               {dcData_Photon_HighDM_nrt,                                          dcMC_Photon_HighDM_nrt},                                            {1, 2}, "", 6,  0,  6, true, doNorm, label_nrt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nj" + histSuffix,                                                 {dcData_Photon_LowDM_nj,                                            dcMC_Photon_LowDM_nj},                                              {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nj_nb0" + histSuffix,                                             {dcData_Photon_LowDM_nj_nb0,                                        dcMC_Photon_LowDM_nj_nb0},                                          {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nj_nb1" + histSuffix,                                             {dcData_Photon_LowDM_nj_nb1,                                        dcMC_Photon_LowDM_nj_nb1},                                          {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nj_nb0_rebin" + histSuffix,                                       {dcData_Photon_LowDM_nj_nb0,                                        dcMC_Photon_LowDM_nj_nb0},                                          {1, 2}, "", xbins_nj_nb0, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nj_nb1_rebin" + histSuffix,                                       {dcData_Photon_LowDM_nj_nb1,                                        dcMC_Photon_LowDM_nj_nb1},                                          {1, 2}, "", xbins_nj_nb1, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_nj" + histSuffix,                                           {dcData_Photon_LowDM_Tight_nj,                                      dcMC_Photon_LowDM_Tight_nj},                                        {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nj" + histSuffix,                                                {dcData_Photon_HighDM_nj,                                           dcMC_Photon_HighDM_nj},                                             {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nj_nb1" + histSuffix,                                            {dcData_Photon_HighDM_nj_nb1,                                       dcMC_Photon_HighDM_nj_nb1},                                         {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nj_nb2" + histSuffix,                                            {dcData_Photon_HighDM_nj_nb2,                                       dcMC_Photon_HighDM_nj_nb2},                                         {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nj_nb3" + histSuffix,                                            {dcData_Photon_HighDM_nj_nb3,                                       dcMC_Photon_HighDM_nj_nb3},                                         {1, 2}, "", maxJets,  minJets,  maxJets, true, doNorm, label_nj, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_mtb" + histSuffix,                                                {dcData_Photon_LowDM_mtb,                                           dcMC_Photon_LowDM_mtb},                                             {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_mtb, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_mtb" + histSuffix,                                               {dcData_Photon_HighDM_mtb,                                          dcMC_Photon_HighDM_mtb},                                            {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_mtb, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_ht" + histSuffix,                                                 {dcData_Photon_LowDM_ht,                                            dcMC_Photon_LowDM_ht},                                              {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_ht_split" + histSuffix,                                           {dcData_Photon_LowDM_ht,                                            dcMC_Photon_LowDM_ht_split},                                        {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_ht" + histSuffix,                                           {dcData_Photon_LowDM_Tight_ht,                                      dcMC_Photon_LowDM_Tight_ht},                                        {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_ht" + histSuffix,                                                {dcData_Photon_HighDM_ht,                                           dcMC_Photon_HighDM_ht},                                             {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_ht_split" + histSuffix,                                          {dcData_Photon_HighDM_ht,                                           dcMC_Photon_HighDM_ht_split},                                       {1, 2}, "", nBins,  minHT, maxHT,        true, doNorm, label_ht, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_met" + histSuffix,                                                {dcData_Photon_LowDM_met,                                           dcMC_Photon_LowDM_met},                                             {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_met_split" + histSuffix,                                          {dcData_Photon_LowDM_met,                                           dcMC_Photon_LowDM_met_split},                                       {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_met" + histSuffix,                                          {dcData_Photon_LowDM_Tight_met,                                     dcMC_Photon_LowDM_Tight_met},                                       {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_met" + histSuffix,                                               {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met},                                            {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_met_split" + histSuffix,                                         {dcData_Photon_HighDM_met,                                          dcMC_Photon_HighDM_met_split},                                      {1, 2}, "", nBins,  minPt, maxPt,        true, doNorm, label_metWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_metphi" + histSuffix,                                             {dcData_Photon_LowDM_metphi,                                        dcMC_Photon_LowDM_metphi},                                          {1, 2}, "", nBins,  minPhi, maxPhi,      true, doNorm, label_metphiWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_metphi" + histSuffix,                                       {dcData_Photon_LowDM_Tight_metphi,                                  dcMC_Photon_LowDM_Tight_metphi},                                    {1, 2}, "", nBins,  minPhi, maxPhi,      true, doNorm, label_metphiWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_metphi" + histSuffix,                                            {dcData_Photon_HighDM_metphi,                                       dcMC_Photon_HighDM_metphi},                                         {1, 2}, "", nBins,  minPhi, maxPhi,      true, doNorm, label_metphiWithPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_PhotonPt" + histSuffix,                                           {dcData_Photon_LowDM_PhotonPt,                                      dcMC_Photon_LowDM_PhotonPt},                                        {1, 2}, "", 36, 220.0, 2020.0,           true, doNorm, label_PhotonPt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_PhotonPt" + histSuffix,                                     {dcData_Photon_LowDM_Tight_PhotonPt,                                dcMC_Photon_LowDM_Tight_PhotonPt},                                  {1, 2}, "", 36, 220.0, 2020.0,           true, doNorm, label_PhotonPt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_PhotonPt" + histSuffix,                                          {dcData_Photon_HighDM_PhotonPt,                                     dcMC_Photon_HighDM_PhotonPt},                                       {1, 2}, "", 36, 220.0, 2020.0,           true, doNorm, label_PhotonPt, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_PhotonEta" + histSuffix,                                          {dcData_Photon_LowDM_PhotonEta,                                     dcMC_Photon_LowDM_PhotonEta},                                       {1, 2}, "", nBins,  minEta, maxEta,      true, doNorm, label_PhotonEta, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_PhotonEta" + histSuffix,                                    {dcData_Photon_LowDM_Tight_PhotonEta,                               dcMC_Photon_LowDM_Tight_PhotonEta},                                 {1, 2}, "", nBins,  minEta, maxEta,      true, doNorm, label_PhotonEta, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_PhotonEta" + histSuffix,                                         {dcData_Photon_HighDM_PhotonEta,                                    dcMC_Photon_HighDM_PhotonEta},                                      {1, 2}, "", nBins,  minEta, maxEta,      true, doNorm, label_PhotonEta, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_dR_GenPhotonGenParton" + histSuffix,                              {dcData_Photon_LowDM_dR_GenPhotonGenParton,                         dcMC_Photon_LowDM_dR_GenPhotonGenParton},                           {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_GenPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_dR_GenPhotonGenParton" + histSuffix,                        {dcData_Photon_LowDM_Tight_dR_GenPhotonGenParton,                   dcMC_Photon_LowDM_Tight_dR_GenPhotonGenParton},                     {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_GenPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_dR_GenPhotonGenParton" + histSuffix,                             {dcData_Photon_HighDM_dR_GenPhotonGenParton,                        dcMC_Photon_HighDM_dR_GenPhotonGenParton},                          {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_GenPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_dR_RecoPhotonGenParton" + histSuffix,                        {dcData_Photon_NoBaseline_dR_RecoPhotonGenParton,                   dcMC_Photon_NoBaseline_dR_RecoPhotonGenParton},                     {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_withQCDcut_dR_RecoPhotonGenParton" + histSuffix,             {dcData_Photon_NoBaseline_dR_RecoPhotonGenParton,                   dcMC_Photon_NoBaseline_withQCDcut_dR_RecoPhotonGenParton},          {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_0to10_dR_RecoPhotonGenParton" + histSuffix,                  {dcData_Photon_NoBaseline_dR_RecoPhotonGenParton,                   dcMC_Photon_NoBaseline_dR_RecoPhotonGenParton},                     {1, 2}, "", nBins, 0.0, 10.0,            true, false, label_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_0to10_withQCDcut_dR_RecoPhotonGenParton" + histSuffix,       {dcData_Photon_NoBaseline_dR_RecoPhotonGenParton,                   dcMC_Photon_NoBaseline_withQCDcut_dR_RecoPhotonGenParton},          {1, 2}, "", nBins, 0.0, 10.0,            true, false, label_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_dR_RecoPhotonGenParton" + histSuffix,                             {dcData_Photon_LowDM_dR_RecoPhotonGenParton,                        dcMC_Photon_LowDM_dR_RecoPhotonGenParton},                          {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_dR_RecoPhotonGenParton" + histSuffix,                       {dcData_Photon_LowDM_Tight_dR_RecoPhotonGenParton,                  dcMC_Photon_LowDM_Tight_dR_RecoPhotonGenParton},                    {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_dR_RecoPhotonGenParton" + histSuffix,                            {dcData_Photon_HighDM_dR_RecoPhotonGenParton,                       dcMC_Photon_HighDM_dR_RecoPhotonGenParton},                         {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_dR_PromptPhotonGenParton" + histSuffix,                      {dcData_Photon_NoBaseline_dR_PromptPhotonGenParton,                 dcMC_Photon_NoBaseline_dR_PromptPhotonGenParton},                   {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_withQCDcut_dR_PromptPhotonGenParton" + histSuffix,           {dcData_Photon_NoBaseline_dR_PromptPhotonGenParton,                 dcMC_Photon_NoBaseline_withQCDcut_dR_PromptPhotonGenParton},        {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_0to10_dR_PromptPhotonGenParton" + histSuffix,                {dcData_Photon_NoBaseline_dR_PromptPhotonGenParton,                 dcMC_Photon_NoBaseline_dR_PromptPhotonGenParton},                   {1, 2}, "", nBins, 0.0, 10.0,            true, false, label_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_0to10_withQCDcut_dR_PromptPhotonGenParton" + histSuffix,     {dcData_Photon_NoBaseline_dR_PromptPhotonGenParton,                 dcMC_Photon_NoBaseline_withQCDcut_dR_PromptPhotonGenParton},        {1, 2}, "", nBins, 0.0, 10.0,            true, false, label_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_dR_PromptPhotonGenParton" + histSuffix,                           {dcData_Photon_LowDM_dR_PromptPhotonGenParton,                      dcMC_Photon_LowDM_dR_PromptPhotonGenParton},                        {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_dR_PromptPhotonGenParton" + histSuffix,                     {dcData_Photon_LowDM_Tight_dR_PromptPhotonGenParton,                dcMC_Photon_LowDM_Tight_dR_PromptPhotonGenParton},                  {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_dR_PromptPhotonGenParton" + histSuffix,                          {dcData_Photon_HighDM_dR_PromptPhotonGenParton,                     dcMC_Photon_HighDM_dR_PromptPhotonGenParton},                       {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_dR_RecoPhotonGenPhoton" + histSuffix,                             {dcData_Photon_LowDM_dR_RecoPhotonGenPhoton,                        dcMC_Photon_LowDM_dR_RecoPhotonGenPhoton},                          {1, 2}, "", nBins, 0.0, 0.2,             true, false, label_dR_RecoPhotonGenPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_dR_RecoPhotonGenPhoton" + histSuffix,                       {dcData_Photon_LowDM_Tight_dR_RecoPhotonGenPhoton,                  dcMC_Photon_LowDM_Tight_dR_RecoPhotonGenPhoton},                    {1, 2}, "", nBins, 0.0, 0.2,             true, false, label_dR_RecoPhotonGenPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_dR_RecoPhotonGenPhoton" + histSuffix,                            {dcData_Photon_HighDM_dR_RecoPhotonGenPhoton,                       dcMC_Photon_HighDM_dR_RecoPhotonGenPhoton},                         {1, 2}, "", nBins, 0.0, 0.2,             true, false, label_dR_RecoPhotonGenPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_min_dR_GenPhotonGenParton" + histSuffix,                          {dcData_Photon_LowDM_min_dR_GenPhotonGenParton,                     dcMC_Photon_LowDM_min_dR_GenPhotonGenParton},                       {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_GenPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_min_dR_GenPhotonGenParton" + histSuffix,                    {dcData_Photon_LowDM_Tight_min_dR_GenPhotonGenParton,               dcMC_Photon_LowDM_Tight_min_dR_GenPhotonGenParton},                 {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_GenPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_min_dR_GenPhotonGenParton" + histSuffix,                         {dcData_Photon_HighDM_min_dR_GenPhotonGenParton,                    dcMC_Photon_HighDM_min_dR_GenPhotonGenParton},                      {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_GenPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_min_dR_RecoPhotonGenParton" + histSuffix,                    {dcData_Photon_NoBaseline_min_dR_RecoPhotonGenParton,               dcMC_Photon_NoBaseline_min_dR_RecoPhotonGenParton},                 {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_withQCDcut_min_dR_RecoPhotonGenParton" + histSuffix,         {dcData_Photon_NoBaseline_min_dR_RecoPhotonGenParton,               dcMC_Photon_NoBaseline_withQCDcut_min_dR_RecoPhotonGenParton},      {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_0to10_min_dR_RecoPhotonGenParton" + histSuffix,              {dcData_Photon_NoBaseline_min_dR_RecoPhotonGenParton,               dcMC_Photon_NoBaseline_min_dR_RecoPhotonGenParton},                 {1, 2}, "", nBins, 0.0, 10.0,            true, false, label_min_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_0to10_withQCDcut_min_dR_RecoPhotonGenParton" + histSuffix,   {dcData_Photon_NoBaseline_min_dR_RecoPhotonGenParton,               dcMC_Photon_NoBaseline_withQCDcut_min_dR_RecoPhotonGenParton},      {1, 2}, "", nBins, 0.0, 10.0,            true, false, label_min_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_min_dR_RecoPhotonGenParton" + histSuffix,                         {dcData_Photon_LowDM_min_dR_RecoPhotonGenParton,                    dcMC_Photon_LowDM_min_dR_RecoPhotonGenParton},                      {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_min_dR_RecoPhotonGenParton" + histSuffix,                   {dcData_Photon_LowDM_Tight_min_dR_RecoPhotonGenParton,              dcMC_Photon_LowDM_Tight_min_dR_RecoPhotonGenParton},                {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_min_dR_RecoPhotonGenParton" + histSuffix,                        {dcData_Photon_HighDM_min_dR_RecoPhotonGenParton,                   dcMC_Photon_HighDM_min_dR_RecoPhotonGenParton},                     {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_RecoPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_min_dR_PromptPhotonGenParton" + histSuffix,                  {dcData_Photon_NoBaseline_min_dR_PromptPhotonGenParton,             dcMC_Photon_NoBaseline_min_dR_PromptPhotonGenParton},               {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_withQCDcut_min_dR_PromptPhotonGenParton" + histSuffix,       {dcData_Photon_NoBaseline_min_dR_PromptPhotonGenParton,             dcMC_Photon_NoBaseline_withQCDcut_min_dR_PromptPhotonGenParton},    {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_0to10_min_dR_PromptPhotonGenParton" + histSuffix,            {dcData_Photon_NoBaseline_min_dR_PromptPhotonGenParton,             dcMC_Photon_NoBaseline_min_dR_PromptPhotonGenParton},               {1, 2}, "", nBins, 0.0, 10.0,            true, false, label_min_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_NoBaseline_0to10_withQCDcut_min_dR_PromptPhotonGenParton" + histSuffix, {dcData_Photon_NoBaseline_min_dR_PromptPhotonGenParton,             dcMC_Photon_NoBaseline_withQCDcut_min_dR_PromptPhotonGenParton},    {1, 2}, "", nBins, 0.0, 10.0,            true, false, label_min_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_min_dR_PromptPhotonGenParton" + histSuffix,                       {dcData_Photon_LowDM_min_dR_PromptPhotonGenParton,                  dcMC_Photon_LowDM_min_dR_PromptPhotonGenParton},                    {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_min_dR_PromptPhotonGenParton" + histSuffix,                 {dcData_Photon_LowDM_Tight_min_dR_PromptPhotonGenParton,            dcMC_Photon_LowDM_Tight_min_dR_PromptPhotonGenParton},              {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_min_dR_PromptPhotonGenParton" + histSuffix,                      {dcData_Photon_HighDM_min_dR_PromptPhotonGenParton,                 dcMC_Photon_HighDM_min_dR_PromptPhotonGenParton},                   {1, 2}, "", nBins, 0.0, 1.0,             true, false, label_min_dR_PromptPhotonGenParton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_min_dR_RecoPhotonGenPhoton" + histSuffix,                         {dcData_Photon_LowDM_min_dR_RecoPhotonGenPhoton,                    dcMC_Photon_LowDM_min_dR_RecoPhotonGenPhoton},                      {1, 2}, "", nBins, 0.0, 0.2,             true, false, label_min_dR_RecoPhotonGenPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_Tight_min_dR_RecoPhotonGenPhoton" + histSuffix,                   {dcData_Photon_LowDM_Tight_min_dR_RecoPhotonGenPhoton,              dcMC_Photon_LowDM_Tight_min_dR_RecoPhotonGenPhoton},                {1, 2}, "", nBins, 0.0, 0.2,             true, false, label_min_dR_RecoPhotonGenPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_min_dR_RecoPhotonGenPhoton" + histSuffix,                        {dcData_Photon_HighDM_min_dR_RecoPhotonGenPhoton,                   dcMC_Photon_HighDM_min_dR_RecoPhotonGenPhoton},                     {1, 2}, "", nBins, 0.0, 0.2,             true, false, label_min_dR_RecoPhotonGenPhoton, label_y_axis));
            vh.push_back(PHS("DataMC_Photon_LowDM_nCRUnitLowDM" + histSuffix,                                       {dcData_Photon_LowDM_nCRUnitLowDM,                                  dcMC_Photon_LowDM_nCRUnitLowDM},                                    {1, 2}, "", max_crunit_low_dm - min_crunit_low_dm,    min_crunit_low_dm,  max_crunit_low_dm,   true, false, "Control Region Unit Low DM",  label_y_axis));
            vh.push_back(PHS("DataMC_Photon_HighDM_nCRUnitHighDM" + histSuffix,                                     {dcData_Photon_HighDM_nCRUnitHighDM,                                dcMC_Photon_HighDM_nCRUnitHighDM},                                  {1, 2}, "", max_crunit_high_dm - min_crunit_high_dm,  min_crunit_high_dm, max_crunit_high_dm,  true, false, "Control Region Unit High DM", label_y_axis));
            
            // dphi
            for (int i = 0; i < 4; i++)
                                                                                                  {
                std::string nameLowDM        = "DataMC_Photon_LowDM_dPhi"        + std::to_string(i+1) + histSuffix;
                std::string nameLowDM_Tight  = "DataMC_Photon_LowDM_Tight_dPhi"  + std::to_string(i+1) + histSuffix;
                std::string nameHighDM       = "DataMC_Photon_HighDM_dPhi"       + std::to_string(i+1) + histSuffix;
                vh.push_back(PHS(nameLowDM,          {dcVecData_Photon_LowDM_dPhi[i],       dcVecMC_Photon_LowDM_dPhi[i]},       {1, 2}, "", nBins,  0.0, maxPhi, true, doNorm, vec_label_dphi[i], label_y_axis));
                vh.push_back(PHS(nameLowDM_Tight,    {dcVecData_Photon_LowDM_Tight_dPhi[i], dcVecMC_Photon_LowDM_Tight_dPhi[i]}, {1, 2}, "", nBins,  0.0, maxPhi, true, doNorm, vec_label_dphi[i], label_y_axis));
                vh.push_back(PHS(nameHighDM,         {dcVecData_Photon_HighDM_dPhi[i],      dcVecMC_Photon_HighDM_dPhi[i]},      {1, 2}, "", nBins,  0.0, maxPhi, true, doNorm, vec_label_dphi[i], label_y_axis));
            }

            // cut flow: name, DataCollection, cutLevels
            // Plotter::CutFlowSummary::CutFlowSummary(std::string n, Plotter::DataCollection ns, std::vector<std::string> cutLevels)
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_Data_Photon_LowDM_met"      + histSuffix,    dcData_Photon_LowDM_met,  CutLevels_Data_Photon_LowDM));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_Data_Photon_HighDM_met"     + histSuffix,    dcData_Photon_HighDM_met, CutLevels_Data_Photon_HighDM));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_MC_Photon_LowDM_met"        + histSuffix,    dcMC_Photon_LowDM_met,    CutLevels_MC_Photon_LowDM));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_MC_Photon_HighDM_met"       + histSuffix,    dcMC_Photon_HighDM_met,   CutLevels_MC_Photon_HighDM));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_Data_Photon_LowDM_All_met"  + histSuffix,    dcData_Photon_LowDM_met,  CutLevels_Data_Photon_LowDM_All));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_Data_Photon_HighDM_All_met" + histSuffix,    dcData_Photon_HighDM_met, CutLevels_Data_Photon_HighDM_All));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_MC_Photon_LowDM_All_met"    + histSuffix,    dcMC_Photon_LowDM_met,    CutLevels_MC_Photon_LowDM_All));
            cutFlowSummaries.emplace_back(Plotter::CutFlowSummary("CutFlow_MC_Photon_HighDM_All_met"   + histSuffix,    dcMC_Photon_HighDM_met,   CutLevels_MC_Photon_HighDM_All));
        }
        // end photon section
    }
    ...
    */
    ...
    // Znunu
    // WARNING: do not apply weights to data
    auto makePDSMET         = [&](const std::string& label, const std::string& cuts = "")                                  {return PDS("Data MET "+label,    fileMap["Data_MET" + yearTag], cuts, ""); };
    ...
    auto makePDSGJets       = [&](const std::string& label, const std::string& cuts = "", const std::string& weights = "") {return PDS("GJets "+label, fileMap["GJets" + yearTag], cuts, weights); };
    auto makePDSTTbar       = [&](const std::string& label, const std::string& cuts = "", const std::string& weights = "") {return PDS("TTbar "+label, fileMap["TTbar" + yearTag], cuts, weights); };
    ...
    if (doSearchBins)
    {
        ...
        for (const auto& JetPtCut : JetPtCuts) 
        {
            ...
            PDC dcMC_ZNuNu_nWs_veto_down("single",              "nWs"               + JetPtCut, {makePDSZnunu("fatjet_veto_down",   "SAT_Pass_Baseline"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc_veto_dn")});
            // TTbar
            ...
            // ZNuNu
            PDC dcMC_ZNuNu_MergedTop_nGenPart_recalc("single",         "FatJet_nGenPart{FatJet_Stop0l=1}", {makePDSZnunu("recalc",             "SAT_Pass_Baseline"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc")});
            PDC dcMC_ZNuNu_MergedTop_nGenPart_top_up("single",         "FatJet_nGenPart{FatJet_Stop0l=1}", {makePDSZnunu("mergedtop_up",       "SAT_Pass_Baseline"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc_top_up")});
            PDC dcMC_ZNuNu_MergedTop_nGenPart_top_down("single",       "FatJet_nGenPart{FatJet_Stop0l=1}", {makePDSZnunu("mergedtop_down",     "SAT_Pass_Baseline"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc_top_dn")});
            // TTbar
            ...
            PDC dcMC_TTbar_MergedTop_nGenPart_top_up("single",         "FatJet_nGenPart{FatJet_Stop0l=1}", {makePDSTTbar("mergedtop_up",       "SAT_Pass_Baseline"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc_top_up")});
            PDC dcMC_TTbar_MergedTop_nGenPart_top_down("single",       "FatJet_nGenPart{FatJet_Stop0l=1}", {makePDSTTbar("mergedtop_down",     "SAT_Pass_Baseline"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc_top_dn")});
            // GJets
            // use photon CR baseline and variable (photon cleaning) 
            //PDC dcMC_GJets_MergedTop_nGenPart_recalc("single",         "FatJet_nGenPart{FatJet_Stop0l=1;!FatJet_matchesPhoton}", {makePDSGJets("recalc",             "SAT_Pass_Baseline_drPhotonCleaned"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc")});
            //PDC dcMC_GJets_MergedTop_nGenPart_top_up("single",         "FatJet_nGenPart{FatJet_Stop0l=1;!FatJet_matchesPhoton}", {makePDSGJets("mergedtop_up",       "SAT_Pass_Baseline_drPhotonCleaned"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc_top_up")});
            //PDC dcMC_GJets_MergedTop_nGenPart_top_down("single",       "FatJet_nGenPart{FatJet_Stop0l=1;!FatJet_matchesPhoton}", {makePDSGJets("mergedtop_down",     "SAT_Pass_Baseline_drPhotonCleaned"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc_top_dn")});
            PDC dcMC_GJets_MergedTop_nGenPart_recalc("single",         "FatJet_nGenPart{FatJet_Stop0l=1}", {makePDSGJets("recalc",             "SAT_Pass_Baseline_drPhotonCleaned"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc")});
            PDC dcMC_GJets_MergedTop_nGenPart_top_up("single",         "FatJet_nGenPart{FatJet_Stop0l=1}", {makePDSGJets("mergedtop_up",       "SAT_Pass_Baseline_drPhotonCleaned"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc_top_up")});
            PDC dcMC_GJets_MergedTop_nGenPart_top_down("single",       "FatJet_nGenPart{FatJet_Stop0l=1}", {makePDSGJets("mergedtop_down",     "SAT_Pass_Baseline_drPhotonCleaned"   + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, "Stop0l_DeepAK8_SFWeight_recalc_top_dn")});
            
            // compare ResolvedTopTotalSF_jetpt30 with Stop0l_ResTopWeight
            // new top tagger SFs in v6 ntuples:
            // - resolved tops:         Stop0l_ResTopWeight
            // - merged tops and Ws:    Stop0l_DeepAK8_SFWeight_recalc
            // resolved top
            ...
            PDS dsT1tttt_gluino2000_lsp100_nmt0_PostProcWeight("T1tttt_gluino2000_lsp100 PostProcWeight",   fileMap["SMS_T1tttt_mGluino2000_mLSP100_fullsim" + yearTag], "SAT_Pass_Baseline" + JetPtCut + ";nMergedTops_jetpt30=0",  "Stop0l_DeepAK8_SFWeight_recalc");
            PDS dsT1tttt_gluino2000_lsp100_nmt1_PostProcWeight("T1tttt_gluino2000_lsp100 PostProcWeight",   fileMap["SMS_T1tttt_mGluino2000_mLSP100_fullsim" + yearTag], "SAT_Pass_Baseline" + JetPtCut + ";nMergedTops_jetpt30=1",  "Stop0l_DeepAK8_SFWeight_recalc");
            PDS dsT1tttt_gluino2000_lsp100_nmt2_PostProcWeight("T1tttt_gluino2000_lsp100 PostProcWeight",   fileMap["SMS_T1tttt_mGluino2000_mLSP100_fullsim" + yearTag], "SAT_Pass_Baseline" + JetPtCut + ";nMergedTops_jetpt30=2",  "Stop0l_DeepAK8_SFWeight_recalc");
            // W
            ...
            PDS dsT1tttt_gluino2000_lsp100_nw0_PostProcWeight("T1tttt_gluino2000_lsp100 PostProcWeight",   fileMap["SMS_T1tttt_mGluino2000_mLSP100_fullsim" + yearTag], "SAT_Pass_Baseline" + JetPtCut + ";nWs_jetpt30=0",  "Stop0l_DeepAK8_SFWeight_recalc");
            PDS dsT1tttt_gluino2000_lsp100_nw1_PostProcWeight("T1tttt_gluino2000_lsp100 PostProcWeight",   fileMap["SMS_T1tttt_mGluino2000_mLSP100_fullsim" + yearTag], "SAT_Pass_Baseline" + JetPtCut + ";nWs_jetpt30=1",  "Stop0l_DeepAK8_SFWeight_recalc");
            PDS dsT1tttt_gluino2000_lsp100_nw2_PostProcWeight("T1tttt_gluino2000_lsp100 PostProcWeight",   fileMap["SMS_T1tttt_mGluino2000_mLSP100_fullsim" + yearTag], "SAT_Pass_Baseline" + JetPtCut + ";nWs_jetpt30=2",  "Stop0l_DeepAK8_SFWeight_recalc");
            // resolved top
            ...
            vh.push_back(PHS("ZNuNu_nResolvedTops",                     {dcMC_ZNuNu_nResolvedTops_nominal, dcMC_ZNuNu_nResolvedTops_up, dcMC_ZNuNu_nResolvedTops_down},             {2, 1}, "",  6, 0, 6,  true, false,  label_nrt,     "Events", true));
            ...
            vh.push_back(PHS("ZNuNu_nMergedTops_topsyst",               {dcMC_ZNuNu_nMergedTops_recalc, dcMC_ZNuNu_nMergedTops_top_up, dcMC_ZNuNu_nMergedTops_top_down},            {2, 1}, "",  6, 0, 6,  true, false,  label_nmt,     "Events", true));
            vh.push_back(PHS("ZNuNu_nMergedTops_vetosyst",              {dcMC_ZNuNu_nMergedTops_recalc, dcMC_ZNuNu_nMergedTops_veto_up, dcMC_ZNuNu_nMergedTops_veto_down},          {2, 1}, "",  6, 0, 6,  true, false,  label_nmt,     "Events", true));
            ...
            vh.push_back(PHS("ZNuNu_nWs_wsyst",                         {dcMC_ZNuNu_nWs_recalc, dcMC_ZNuNu_nWs_w_up, dcMC_ZNuNu_nWs_w_down},                                        {2, 1}, "",  6, 0, 6,  true, false,  label_nw,      "Events", true));
            vh.push_back(PHS("ZNuNu_nWs_vetosyst",                      {dcMC_ZNuNu_nWs_recalc, dcMC_ZNuNu_nWs_veto_up, dcMC_ZNuNu_nWs_veto_down},                                  {2, 1}, "",  6, 0, 6,  true, false,  label_nw,      "Events", true));
            vh.push_back(PHS("TTbar_nResolvedTops",                     {dcMC_TTbar_nResolvedTops_nominal, dcMC_TTbar_nResolvedTops_up, dcMC_TTbar_nResolvedTops_down},             {2, 1}, "",  6, 0, 6,  true, false,  label_nrt,     "Events", true));
            ...
            vh.push_back(PHS("TTbar_nMergedTops_topsyst",               {dcMC_TTbar_nMergedTops_recalc, dcMC_TTbar_nMergedTops_top_up, dcMC_TTbar_nMergedTops_top_down},            {2, 1}, "",  6, 0, 6,  true, false,  label_nmt,     "Events", true));
            vh.push_back(PHS("TTbar_nMergedTops_vetosyst",              {dcMC_TTbar_nMergedTops_recalc, dcMC_TTbar_nMergedTops_veto_up, dcMC_TTbar_nMergedTops_veto_down},          {2, 1}, "",  6, 0, 6,  true, false,  label_nmt,     "Events", true));
            ...
            // nGenPart
            vh.push_back(PHS("ZNuNu_MergedTop_nGenPart_recalc_nmteq0",  {dcMC_ZNuNu_MergedTop_nGenPart_recalc, dcMC_ZNuNu_MergedTop_nGenPart_top_up, dcMC_ZNuNu_MergedTop_nGenPart_top_down},   {2, 1}, "nMergedTops_jetpt30=0",   11, 0, 11,  true, false,  "MergedTop_nGenPart, nmt=0",     "Events", true));
            vh.push_back(PHS("ZNuNu_MergedTop_nGenPart_recalc_nmteq1",  {dcMC_ZNuNu_MergedTop_nGenPart_recalc, dcMC_ZNuNu_MergedTop_nGenPart_top_up, dcMC_ZNuNu_MergedTop_nGenPart_top_down},   {2, 1}, "nMergedTops_jetpt30=1",   11, 0, 11,  true, false,  "MergedTop_nGenPart, nmt=1",     "Events", true));
            vh.push_back(PHS("ZNuNu_MergedTop_nGenPart_recalc_nmtgeq2", {dcMC_ZNuNu_MergedTop_nGenPart_recalc, dcMC_ZNuNu_MergedTop_nGenPart_top_up, dcMC_ZNuNu_MergedTop_nGenPart_top_down},   {2, 1}, "nMergedTops_jetpt30>=2",  11, 0, 11,  true, false,  "MergedTop_nGenPart, nmt>=2",    "Events", true));
            vh.push_back(PHS("TTbar_MergedTop_nGenPart_recalc_nmteq0",  {dcMC_TTbar_MergedTop_nGenPart_recalc, dcMC_TTbar_MergedTop_nGenPart_top_up, dcMC_TTbar_MergedTop_nGenPart_top_down},   {2, 1}, "nMergedTops_jetpt30=0",   11, 0, 11,  true, false,  "MergedTop_nGenPart, nmt=0",     "Events", true));
            vh.push_back(PHS("TTbar_MergedTop_nGenPart_recalc_nmteq1",  {dcMC_TTbar_MergedTop_nGenPart_recalc, dcMC_TTbar_MergedTop_nGenPart_top_up, dcMC_TTbar_MergedTop_nGenPart_top_down},   {2, 1}, "nMergedTops_jetpt30=1",   11, 0, 11,  true, false,  "MergedTop_nGenPart, nmt=1",     "Events", true));
            vh.push_back(PHS("TTbar_MergedTop_nGenPart_recalc_nmtgeq2", {dcMC_TTbar_MergedTop_nGenPart_recalc, dcMC_TTbar_MergedTop_nGenPart_top_up, dcMC_TTbar_MergedTop_nGenPart_top_down},   {2, 1}, "nMergedTops_jetpt30>=2",  11, 0, 11,  true, false,  "MergedTop_nGenPart, nmt>=2",    "Events", true));
            vh.push_back(PHS("GJets_MergedTop_nGenPart_recalc_nmteq0",  {dcMC_GJets_MergedTop_nGenPart_recalc, dcMC_GJets_MergedTop_nGenPart_top_up, dcMC_GJets_MergedTop_nGenPart_top_down},   {2, 1}, "nMergedTops_jetpt30=0",   11, 0, 11,  true, false,  "MergedTop_nGenPart, nmt=0",     "Events", true));
            vh.push_back(PHS("GJets_MergedTop_nGenPart_recalc_nmteq1",  {dcMC_GJets_MergedTop_nGenPart_recalc, dcMC_GJets_MergedTop_nGenPart_top_up, dcMC_GJets_MergedTop_nGenPart_top_down},   {2, 1}, "nMergedTops_jetpt30=1",   11, 0, 11,  true, false,  "MergedTop_nGenPart, nmt=1",     "Events", true));
            vh.push_back(PHS("GJets_MergedTop_nGenPart_recalc_nmtgeq2", {dcMC_GJets_MergedTop_nGenPart_recalc, dcMC_GJets_MergedTop_nGenPart_top_up, dcMC_GJets_MergedTop_nGenPart_top_down},   {2, 1}, "nMergedTops_jetpt30>=2",  11, 0, 11,  true, false,  "MergedTop_nGenPart, nmt>=2",    "Events", true));
            // select search bins:
            vh.push_back(PHS("ZNuNu_MergedTop_nGenPart_recalc_sb69",     {dcMC_ZNuNu_MergedTop_nGenPart_recalc, dcMC_ZNuNu_MergedTop_nGenPart_top_up, dcMC_ZNuNu_MergedTop_nGenPart_top_down},   {2, 1}, "nSearchBinHighDM_jetpt30=69",   11, 0, 11,  true, false,  "MergedTop_nGenPart, SB=69",  "Events", true));
            vh.push_back(PHS("ZNuNu_MergedTop_nGenPart_recalc_sb105",    {dcMC_ZNuNu_MergedTop_nGenPart_recalc, dcMC_ZNuNu_MergedTop_nGenPart_top_up, dcMC_ZNuNu_MergedTop_nGenPart_top_down},   {2, 1}, "nSearchBinHighDM_jetpt30=105",  11, 0, 11,  true, false,  "MergedTop_nGenPart, SB=105", "Events", true));
            vh.push_back(PHS("ZNuNu_MergedTop_nGenPart_recalc_sb153",    {dcMC_ZNuNu_MergedTop_nGenPart_recalc, dcMC_ZNuNu_MergedTop_nGenPart_top_up, dcMC_ZNuNu_MergedTop_nGenPart_top_down},   {2, 1}, "nSearchBinHighDM_jetpt30=153",  11, 0, 11,  true, false,  "MergedTop_nGenPart, SB=153", "Events", true));
            vh.push_back(PHS("ZNuNu_MergedTop_nGenPart_recalc_sb161",    {dcMC_ZNuNu_MergedTop_nGenPart_recalc, dcMC_ZNuNu_MergedTop_nGenPart_top_up, dcMC_ZNuNu_MergedTop_nGenPart_top_down},   {2, 1}, "nSearchBinHighDM_jetpt30=161",  11, 0, 11,  true, false,  "MergedTop_nGenPart, SB=161", "Events", true));
            vh.push_back(PHS("TTbar_MergedTop_nGenPart_recalc_sb69",     {dcMC_TTbar_MergedTop_nGenPart_recalc, dcMC_TTbar_MergedTop_nGenPart_top_up, dcMC_TTbar_MergedTop_nGenPart_top_down},   {2, 1}, "nSearchBinHighDM_jetpt30=69",   11, 0, 11,  true, false,  "MergedTop_nGenPart, SB=69",  "Events", true));
            vh.push_back(PHS("TTbar_MergedTop_nGenPart_recalc_sb105",    {dcMC_TTbar_MergedTop_nGenPart_recalc, dcMC_TTbar_MergedTop_nGenPart_top_up, dcMC_TTbar_MergedTop_nGenPart_top_down},   {2, 1}, "nSearchBinHighDM_jetpt30=105",  11, 0, 11,  true, false,  "MergedTop_nGenPart, SB=105", "Events", true));
            vh.push_back(PHS("TTbar_MergedTop_nGenPart_recalc_sb153",    {dcMC_TTbar_MergedTop_nGenPart_recalc, dcMC_TTbar_MergedTop_nGenPart_top_up, dcMC_TTbar_MergedTop_nGenPart_top_down},   {2, 1}, "nSearchBinHighDM_jetpt30=153",  11, 0, 11,  true, false,  "MergedTop_nGenPart, SB=153", "Events", true));
            vh.push_back(PHS("TTbar_MergedTop_nGenPart_recalc_sb161",    {dcMC_TTbar_MergedTop_nGenPart_recalc, dcMC_TTbar_MergedTop_nGenPart_top_up, dcMC_TTbar_MergedTop_nGenPart_top_down},   {2, 1}, "nSearchBinHighDM_jetpt30=161",  11, 0, 11,  true, false,  "MergedTop_nGenPart, SB=161", "Events", true));
            // T1tttt
            ...
            if (doSystematics)
            {
                ...
                {
                    std::string histSuffixSyst                  = "_metres_syst_"            + met.first + JetPtCut;
                    std::string SAT_Pass_lowDM_met              = "SAT_Pass_lowDM"           + JetPtCut  + met.second;
                    std::string SAT_Pass_highDM_met             = "SAT_Pass_highDM"          + JetPtCut  + met.second;
                    ...
                    // ZNuNu MC in validation and search bins
                    PDC dcMC_ZNuNu_nValidationBin_LowDM("data",             "nValidationBinLowDM"           + JetPtCut + met.second, {makePDSZnunu("Validation Bin Low DM",              SAT_Pass_lowDM_met              + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nValidationBin_LowDM_HighMET("data",     "nValidationBinLowDMHighMET"    + JetPtCut + met.second, {makePDSZnunu("Validation Bin Low DM High MET",     SAT_Pass_lowDM_mid_dPhi_met     + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nValidationBin_HighDM("data",            "nValidationBinHighDM"          + JetPtCut + met.second, {makePDSZnunu("Validation Bin High DM",             SAT_Pass_highDM_mid_dPhi_met    + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nValidationBin_LowDM_METStudy("data",    "nValidationBinLowDM_METStudy"  + JetPtCut + met.second, {makePDSZnunu("Validation Bin Low DM MET Study",    SAT_Pass_lowDM_mid_dPhi_met     + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nValidationBin_HighDM_METStudy("data",   "nValidationBinHighDM_METStudy" + JetPtCut + met.second, {makePDSZnunu("Validation Bin High DM MET Study",   SAT_Pass_highDM_mid_dPhi_met    + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nSearchBin_LowDM("data",                 "nSearchBinLowDM"               + JetPtCut + met.second, {makePDSZnunu("Search Bin Low DM",                  SAT_Pass_lowDM_met              + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nSearchBin_HighDM("data",                "nSearchBinHighDM"              + JetPtCut + met.second, {makePDSZnunu("Search Bin High DM",                 SAT_Pass_highDM_met             + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nSRUnit_LowDM("data",                    "nSRUnitLowDM"                  + JetPtCut + met.second, {makePDSZnunu("Search Region Unit Low DM",          SAT_Pass_lowDM_met              + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nSRUnit_HighDM("data",                   "nSRUnitHighDM"                 + JetPtCut + met.second, {makePDSZnunu("Search Region Unit High DM",         SAT_Pass_highDM_met             + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    // ZNuNu MC in validation and search bins
                    vh.push_back(PHS("ZNuNu_nValidationBin_LowDM" + histSuffixSyst,             {dcMC_ZNuNu_nValidationBin_LowDM},              {1, 1}, "", max_vb_low_dm - min_vb_low_dm,                      min_vb_low_dm,              max_vb_low_dm,              false, false,  "Validation Bin Low DM", "Events", true));
                    vh.push_back(PHS("ZNuNu_nValidationBin_LowDM_HighMET" + histSuffixSyst,     {dcMC_ZNuNu_nValidationBin_LowDM_HighMET},      {1, 1}, "", max_vb_low_dm_high_met - min_vb_low_dm_high_met,    min_vb_low_dm_high_met,     max_vb_low_dm_high_met,     false, false,  "Validation Bin Low DM High MET", "Events", true));
                    vh.push_back(PHS("ZNuNu_nValidationBin_HighDM" + histSuffixSyst,            {dcMC_ZNuNu_nValidationBin_HighDM},             {1, 1}, "", max_vb_high_dm - min_vb_high_dm,                    min_vb_high_dm,             max_vb_high_dm,             false, false,  "Validation Bin High DM", "Events", true));
                ...
                {
                    std::string histSuffixSyst                  = "_jes_syst_"               + jec.first + JetPtCut;
                    std::string SAT_Pass_lowDM_jec              = "SAT_Pass_lowDM"           + JetPtCut  + jec.second;
                    std::string SAT_Pass_highDM_jec             = "SAT_Pass_highDM"          + JetPtCut  + jec.second;
                    ...
                    // ZNuNu MC in validation and search bins
                    PDC dcMC_ZNuNu_nValidationBin_LowDM("data",             "nValidationBinLowDM"           + JetPtCut + jec.second, {makePDSZnunu("Validation Bin Low DM",              SAT_Pass_lowDM_jec           + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nValidationBin_LowDM_HighMET("data",     "nValidationBinLowDMHighMET"    + JetPtCut + jec.second, {makePDSZnunu("Validation Bin Low DM High MET",     SAT_Pass_lowDM_mid_dPhi_jec  + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nValidationBin_HighDM("data",            "nValidationBinHighDM"          + JetPtCut + jec.second, {makePDSZnunu("Validation Bin High DM",             SAT_Pass_highDM_mid_dPhi_jec + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nValidationBin_LowDM_METStudy("data",    "nValidationBinLowDM_METStudy"  + JetPtCut + jec.second, {makePDSZnunu("Validation Bin Low DM MET Study",    SAT_Pass_lowDM_mid_dPhi_jec  + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nValidationBin_HighDM_METStudy("data",   "nValidationBinHighDM_METStudy" + JetPtCut + jec.second, {makePDSZnunu("Validation Bin High DM MET Study",   SAT_Pass_highDM_mid_dPhi_jec + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nSearchBin_LowDM("data",                 "nSearchBinLowDM"               + JetPtCut + jec.second, {makePDSZnunu("Search Bin Low DM",                  SAT_Pass_lowDM_jec           + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nSearchBin_HighDM("data",                "nSearchBinHighDM"              + JetPtCut + jec.second, {makePDSZnunu("Search Bin High DM",                 SAT_Pass_highDM_jec          + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nSRUnit_LowDM("data",                    "nSRUnitLowDM"                  + JetPtCut + jec.second, {makePDSZnunu("Search Region Unit Low DM",          SAT_Pass_lowDM_jec           + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    PDC dcMC_ZNuNu_nSRUnit_HighDM("data",                   "nSRUnitHighDM"                 + JetPtCut + jec.second, {makePDSZnunu("Search Region Unit High DM",         SAT_Pass_highDM_jec          + Flag_ecalBadCalibFilter + semicolon_HEMVeto, ZNuNuWeights)});
                    // ZNuNu MC in validation and search bins
                    vh.push_back(PHS("ZNuNu_nValidationBin_LowDM" + histSuffixSyst,             {dcMC_ZNuNu_nValidationBin_LowDM},              {1, 1}, "", max_vb_low_dm - min_vb_low_dm,                      min_vb_low_dm,              max_vb_low_dm,              false, false,  "Validation Bin Low DM", "Events", true));
                    vh.push_back(PHS("ZNuNu_nValidationBin_LowDM_HighMET" + histSuffixSyst,     {dcMC_ZNuNu_nValidationBin_LowDM_HighMET},      {1, 1}, "", max_vb_low_dm_high_met - min_vb_low_dm_high_met,    min_vb_low_dm_high_met,     max_vb_low_dm_high_met,     false, false,  "Validation Bin Low DM High MET", "Events", true));
                    vh.push_back(PHS("ZNuNu_nValidationBin_HighDM" + histSuffixSyst,            {dcMC_ZNuNu_nValidationBin_HighDM},             {1, 1}, "", max_vb_high_dm - min_vb_high_dm,                    min_vb_high_dm,             max_vb_high_dm,             false, false,  "Validation Bin High DM", "Events", true));
                ...
                for (const auto& element : systematics_json.items())
                {
                    ...
                    std::string syst_up         = systematics_json[syst]["up"];
                    std::string syst_down       = systematics_json[syst]["down"];
                    size_t pos = ZNuNuWeights.find(syst_nominal);
                    size_t len = syst_nominal.length();
                    if (pos != std::string::npos)
                    {
                        std::string ZNuNuWeights_up     = ZNuNuWeights;
                        ...
                        for (const auto& w : weightMap)
                        {
                            ...
                            PDC dcMC_ZNuNu_nValidationBin_LowDM_HighMET_njetWeight("data", "nValidationBinLowDMHighMET"    + JetPtCut, {makePDSZnunu("Validation Bin Low DM High MET", "SAT_Pass_lowDM_mid_dPhi"  + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, w.second + ";njetWeight_Electron_LowDM")});
                            PDC dcMC_ZNuNu_nValidationBin_HighDM_njetWeight("data",        "nValidationBinHighDM"          + JetPtCut, {makePDSZnunu("Validation Bin High DM",         "SAT_Pass_highDM_mid_dPhi" + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, w.second + ";njetWeight_Electron_HighDM")});
                            PDC dcMC_ZNuNu_nSearchBin_LowDM_njetWeight("data",             "nSearchBinLowDM"               + JetPtCut, {makePDSZnunu("Search Bin Low DM",              "SAT_Pass_lowDM"           + JetPtCut + Flag_ecalBadCalibFilter + semicolon_HEMVeto, w.second + ";njetWeight_Electron_LowDM")});
