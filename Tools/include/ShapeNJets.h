...
namespace plotterFunctions
{
    ...
    public:
        ShapeNJets()
        {
            ...
            if(file_exists && f)
            {
               hmap["Electron_HighDM"]  = static_cast<TH1D*>(f->Get("njets_shape_2016_Combine_HighDM_Loose"));
               hmap["Electron_LowDM"]  = static_cast<TH1D*>(f->Get("njets_shape_2016_Combine_LowDM_Loose"));
               hmap["Muon_HighDM"]  = static_cast<TH1D*>(f->Get("njets_shape_2016_Combine_HighDM_Loose"));
               hmap["Muon_LowDM"]  = static_cast<TH1D*>(f->Get("njets_shape_2016_Combine_LowDM_Loose"));
                f->Close();
                delete f;
            }
            else
